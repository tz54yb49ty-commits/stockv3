# N4 Worker Bounded Smoke Post Review

Result: `POST_REVIEW_PASS`

Gate: `N4_WORKER_BOUNDED_SMOKE_POST_REVIEW_GATE`

Generated at: `2026-06-10T08:13:35+08:00`
...
| selected source events | 5 |
| selected source events pending | 5 |
| selected source events not pending | 0 |

The selected 5 inbox rows still point to pending `MarketSnapshotUpdated` source events. N3 facts and N3 outbox status are preserved.

## N4 Semantic Proof

This smoke is a scoped consumption / inbox / checkpoint probe only:
...
- future delete scope is limited to scoped smoke rows
...
The scoped N4 bounded worker smoke probe can be marked complete.
...
`N4_WORKER_BOUNDED_SMOKE_EXPANDED_SCOPE_READINESS_GATE`

Optional cleanup gate if scoped smoke rows should be removed:

`N4_WORKER_BOUNDED_SMOKE_ROLLBACK_FINAL_GATE_REVIEW`
