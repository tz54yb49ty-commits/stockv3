...
10. The compiler may emit a runtime plan only for
    ...
    `policy_id=n4_lifecycle_deactivation_state_columns_controlled_promotion_v1`, or
    `policy_id=n4_lifecycle_inactive_mark_recovery_v1`, or
    `policy_id=runtime_hot_cleanup_archive_gated_disk_governance_v1`, and
    only after every rule in the corresponding named-policy section is
    structurally satisfied. The compiler still performs no
    runtime action.
...
`n4_lifecycle_inactive_mark_recovery_v1` compiles only the exact linear chain
`policy -> rollback_restore -> fixed_lifecycle -> typed_columns ->
fixed_code_rollback`. The frozen `cadbe91c` rollback is content evidence only;
all execution commits are prepared after the policy commit by an independent
`N4_trigger` task. The corrected lifecycle must emit inactive current
`trigger_mark_candidate=normal`, retain the former value in
`previous_trigger_mark_candidate`, and preserve projection `false/none`.

Each execution request compiles exactly one phase: stable restore, corrected
promotion, or corrected code-only rollback. A phase requires two exact
bootouts, state-driven absence, one ff-only merge and two original-plist
bootstraps. Combining phases, automatic continuation/rollback, retry,
kickstart, manual execute, schema/constraint changes, DB/message/history,
N2/N3/N5/N6 or trading work compiles to `REJECT`. This governance definition
gate cannot execute the policy.

The current policy revision additionally compiles only after the frozen first
restore attempt is proven to have failed on `.git/ORIG_HEAD.lock` permission
before any ref/tree/index/tracked-file mutation, with both original plists
restored. The prior four-commit chain is content evidence only. An independent
`N4_trigger` task must regenerate the same four patch hashes from the current
policy commit. Every later runtime phase must verify escalated Git metadata
write authority before bootout; a non-authorized merge capability probe or
reuse of the prior target compiles to `REJECT`.
...
`runtime_hot_cleanup_archive_gated_disk_governance_v1` compiles exactly one
selected phase. `cleanup_scheduler_quiesce` contains one exact cleanup-label
bootout and absence wait. `archive_verified_local_reclaim` consumes only one
SHA-bound `LocalArtifactArchiveManifest.v1` exact allowlist, revalidates every
source/archive entry, unlinks only exact regular-file paths in the frozen
family/date order, journals every unlink, measures `df` after each date batch,
and stops at 250 GiB. `time_machine_snapshot_fallback` is available only after
reclaim exhaustion below target and names only frozen purgeable Time Machine
local snapshots. `cleanup_scheduler_archive_gated_restore` permits one exact
bootstrap only after the plist is verified archive-required. Combining phases,
archive creation, database work, glob/recursive deletion, business-service
operation, manual cleanup replay, retry, Release/Codex/old-repository work, or
the policy-definition session requesting execution compiles to `REJECT`.

### 4.18 Windows Rebuild W0 Bounded Compilation

The compiler recognizes exactly one Windows W0 host-governance policy:
...
Compilation is default-deny. The request must be a later independent
`runtime_control` session with explicit current-request authorization and must
select exactly one of these phase modes with exactly one attempt and zero
automatic retry:
...
The phases are mutually exclusive and ordered. The shutdown phase compiles
only after a frozen `RESTART_REQUIRED` result and complete pre-shutdown
evidence from the prepare phase. The policy-definition or policy-modification
session, a combined phase, a second attempt, retry, or reverse edge fails
compilation before Kernel evaluation.

The prepare DAG is strictly one-way:
...
PLAN exact phase, identities, versions, paths, resources and attempt budget
  ...
  -> VERIFY every required post-evidence item, zero Mac imports, zero business
            ...
            all dynamically frozen AshareV3 tasks disabled, exact PostgreSQL
            installer hash/signer/service/account/service-logon-only rights,
            secret redaction audit and exact D ACL, Python311
...
The native-control shutdown DAG is strictly one-way:
...
The compiler must preserve every Kernel forbidden constraint. Tushare,
Mootdx, Mac dump/record/source_version/evidence import, N1-N6, NAS, business
schema/data, Scheduler creation/enable/delete, legacy PostgreSQL uninstall or
...
### Windows W0 PostgreSQL ReturnValue 22 recovery compilation

`w0_postgresql_virtual_identity_22_recovery` compiles only from v6 commit
`0a64eb665433483a69e9134c222a1dabc03c1da2`, tree
`0f97f27c5a43d976e73f025e20d6b355f6ece494`, with CIM ReturnValue 22,
zero service starts and no identity change. It requires a fresh read-only
Stopped/NetworkService/UNRESTRICTED/no-listener precheck, then permits one exact
`sc.exe config postgresql-x64-16 obj= "NT SERVICE\postgresql-x64-16"` process
argument vector with `password=` omitted so ChangeServiceConfig receives NULL.
It verifies StartName before one start and applies the existing final checks.
