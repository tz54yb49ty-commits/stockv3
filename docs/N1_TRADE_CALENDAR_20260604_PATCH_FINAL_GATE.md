# N1 Trade Calendar 20260604 Patch Final Gate
...
calendar_row = is_open=true, prev=20260603, next=20260605
```

## Live Baseline

```text
common_trade_calendar(20260604) = 0
active trade_calendar SSE:20260604 = 0
batch_conflict = 0
quality_conflict = 0
N1 source fact rows for 20260604 = 0
```
...
PYTHONPATH=src:scripts python3 scripts/run_trade_calendar_patch_once.py \
  ...
  --trade-date 20260604 \
  --expected-prev-trade-date 20260603 \
  --fallback-next-trade-date 20260605 \
  --source-batch-id trade_calendar_20260604_patch_v1 \
  --source-version trade_calendar_20260604_patch_v1 \
  --json-report-path docs/N1_trade_calendar_20260604_patch_preflight.json \
  --markdown-report-path docs/N1_TRADE_CALENDAR_20260604_PATCH_PREFLIGHT.md \
  --rollback-sql-path sql/N1_trade_calendar_20260604_patch_rollback.sql \
  --execute \
  --user-confirmed \
...
```text
1. Confirm common_trade_calendar(20260604) exists and is_open=true.
2. Confirm prev/next = 20260603/20260605.
3. Confirm active source version SSE:20260604 exists.
4. Confirm no daily facts, outbox, inbox, checkpoint, N2/N3/N4/N5/N6 writes.
5. Re-run 20260605 calendar patch preflight.
6. Do not enter official daily automatically.
```
