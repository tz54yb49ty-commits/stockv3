...
### T0.DISK. Disk governance Gate 0

状态：policy defined / execution pending；persistent repair code-only implemented

目标：以 `runtime_control` 定义 archive-gated 磁盘治理边界，撤销
`direct-delete-no-archive` 和 runtime-control 数据库 DELETE，并为连续自然运行
建立独立 N1 archive-only daily handoff；本治理定义任务不执行归档、删除、数据库
或 LaunchAgent 操作。

已完成：

- Kernel policy：`runtime_hot_cleanup_archive_gated_disk_governance_v1`。
- 四个互斥阶段：cleanup scheduler quiesce、archive-verified local reclaim、
  Time Machine snapshot fallback、archive-gated scheduler restore。
- exact cleanup label/plist、MacRaid 两类 archive root、250 GiB `df` 停止条件。
- `common_trade_calendar` 权威的当前交易日 + 前 5 个已完成交易日保留集合。
- `LocalArtifactArchiveManifest.v1` 逐文件 identity/hash/restore-proof 最小合同。
- 新增 `n1_local_artifact_archive_daily_bounded_install_v1`，只允许独立安装 exact
  23:00 N1 archive-only label，不允许定义会话安装或运行。
- 冻结 `LocalArtifactArchiveCurrentPointer.v1` exact path/schema；后续 01:00
  cleanup restore 必须使用 current pointer + local-only。
- 2026-08-23 失败根因已冻结：20260814 新增 38,829 个未归档候选；数据库
  20260824 future/current lineage 必须从 cleanup plan 排除。
- 禁止 broad/glob delete、symlink、active/retained lineage、数据库跨层删除、自动
  retry、手工 cleanup replay、业务 LaunchAgent、old-dirty repo、Release、Codex
  session 和 `com.apple.os.update*` snapshot。

未完成 / 不授权：

- N1 daily archive runner/current pointer publisher 已完成 code-only 实现；尚未安装 N1 label。
- cleanup pointer/local-only、future-date guard 与 plist plan 已完成 code-only 实现；尚未修改或恢复 live cleanup plist。
- 本次治理定义未执行 archive、cleanup、数据库写入或 LaunchAgent 操作。
- 未进入 N3 blob/reference、N1 post-close v2、N3/N6 DB retention 或次级治理。

下一 gate：独立 `runtime_control` 安装 exact N1 23:00 LaunchAgent；不得运行 archive、
不得 kickstart/retry，不得修改 cleanup plist、业务服务或数据库。
