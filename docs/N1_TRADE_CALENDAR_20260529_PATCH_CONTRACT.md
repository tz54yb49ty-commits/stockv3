# N1 Trade Calendar 20260529 Patch Contract
...
Patch `common_trade_calendar` for `20260529` and activate the `SSE:20260529`
`trade_calendar` source version. This unblocks the 20260528 N1 daily
incremental gate that requires the next trading day detail row to be explicit.
...
```text
Use common_trade_calendar 20260528.next_trade_date=20260529
...
```bash
PYTHONPATH=src python3 scripts/run_trade_calendar_patch_20260529_once.py \
  --trade-date 20260529 \
  --execute \
  --user-confirmed \
  --postgres-commit-enabled
...
```text
sql/N1_trade_calendar_20260529_patch_rollback.sql
```

Rollback only clears this calendar patch by
`source_batch_id/source_version/trade_date=20260529/scope_key=SSE:20260529`
and deletes or restores the `SSE:20260529` active source version. It does not
touch 20260528 calendar, daily fact, condition source, Parquet, outbox, or
