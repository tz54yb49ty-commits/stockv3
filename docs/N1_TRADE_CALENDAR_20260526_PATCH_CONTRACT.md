# N1 Trade Calendar 20260526 Patch Contract
...
补齐 `common_trade_calendar` 中 `20260526` 的交易日明细，并激活 `SSE:20260526` 的 `trade_calendar` source version，以解除 20260526 daily incremental 的 calendar blocker。
...
```text
依据 common_trade_calendar 中 20260525.next_trade_date=20260526
...
```bash
PYTHONPATH=src python3 scripts/run_trade_calendar_patch_20260526_once.py \
  --trade-date 20260526 \
  --execute \
  --user-confirmed \
  --postgres-commit-enabled
...
```text
sql/N1_trade_calendar_20260526_patch_rollback.sql
```

Rollback 只允许按 `source_batch_id/source_version/trade_date=20260526` 清理本次 calendar patch，并删除或恢复 `SSE:20260526` 的 active source version。不碰 daily fact、Parquet、outbox、N2/N3/N4/N5/N6。
