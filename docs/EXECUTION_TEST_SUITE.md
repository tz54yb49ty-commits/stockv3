...
### 6.22D N4 Lifecycle Inactive Mark Recovery Policy

`policy_id=n4_lifecycle_inactive_mark_recovery_v1`

Static tests bind Active `49fd0a65`, stable `ae05d7f8`, content-only rollback
`cadbe91c`, their trees and rollback patch hash, the exact eight N4 paths, and
the two original plist path/SHA pairs. They require the direct-parent chain
`policy -> rollback_restore -> fixed_lifecycle -> typed_columns ->
fixed_code_rollback` and require the code rollback N4 blobs to equal the
rollback-restore blobs.

Tests accept inactive current `trigger_mark_candidate=normal` only, retain the
prior candidate in `previous_trigger_mark_candidate`, and require projection
`false/none`. `none`, NULL, schema/constraint changes and event-structure
changes reject. Each runtime request may select exactly one phase and exactly
one ff-only merge. Phase combination, automatic progression/rollback, retry,
manual execute, DB/message/history or cross-layer operations reject. The
governance-definition gate cannot execute.

The current revision additionally freezes the first restore attempt's exact
permission-error hash, unchanged Active HEAD/tree/index/tracked state, one
bootout/bootstrap per label, and zero repository/business mutation. Tests
require a newly generated chain with the same four patch hashes and verified
Git metadata write authority before bootout. They reject prior-target reuse,
non-authorized merge probes, hidden prior-attempt evidence and runtime from the
definition request.
...
### Windows rebuild W0 bounded policy matrix

Static tests for `windows_rebuild_w0_bounded_v1` must first preserve the
historical five-way AGENTS/Compiler/Kernel/RuntimeGate/Sandbox consistency
check, then prove six-way consistency when Trace is included, and finally
require this Test Suite as the seventh full-chain control document. Missing
any document, policy id, phase, exact resource or fail-closed constraint must
fail before runtime authority can be considered. AGENTS and Kernel policy JSON
must remain identical; all other documents must recognize the same policy,
general Windows runtime must remain `REJECT`, and Sandbox PASS must not mean
execution.

Tests must keep `w0_prepare_and_mutate` and `wsl_shutdown_native_control`
separate, mutually exclusive, one attempt each and ordered only through a
sealed `RESTART_REQUIRED` handoff. Trace tests must require append-only exact
attempt/mutation counts, before/after evidence, forbidden=0, native shutdown,
reconnect proof, C visible/D absent and fail-closed incomplete evidence.

Exact negative-boundary tests must reject missing or drifted identity, ACL,
path, service, scheduled-task, installer, empty-cluster, loopback, TdxW/17709
or WSL evidence; a path conflict; task deletion/creation/enable; legacy service
uninstall/file mutation; unexpected D access; recursive delete; overwrite;
retry/phase combination; current WSL/SSH self-disconnect; and any operation
outside the exact allowlist. Tests must also prove zero N1-N6 runtime or data
