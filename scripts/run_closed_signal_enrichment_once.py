#!/usr/bin/env python3
"""Execute N3-C2B closed signal enrichment once.

This runner is bounded and requires both ``--execute`` and
``--user-confirmed``. It writes only C2B run metadata, quality rows, and
stock/index/board closed signal enrichment facts.
"""
...
from ashare_v3.market.closed_signal_enrichment_execute import (
    DEFAULT_C2B_CONTRACT_JSON_PATH,
    DEFAULT_C2B_DRY_RUN_JSON_PATH,
    DEFAULT_C2B_JSON_REPORT_PATH,
    DEFAULT_C2B_MD_REPORT_PATH,
    DEFAULT_C2B_PREFLIGHT_JSON_PATH,
    DEFAULT_C2B_ROLLBACK_SQL_PATH,
    ClosedSignalEnrichmentExecuteError,
    run_closed_signal_enrichment_execute,
)
...
def main() -> int:
    ...
    try:
        report = run_closed_signal_enrichment_execute(
            dsn=args.dsn,
        ...
        blocked = {
            "result": "BLOCKED",
            "stage": "N3-C2B-closed-signal-enrichment-execute",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Execute N3-C2B closed signal enrichment run-once.")
    parser.add_argument("--dsn", default=os.environ.get("ASHARE_V3_POSTGRES_DSN", DEFAULT_DSN))
    parser.add_argument("--contract-path", default=DEFAULT_C2B_CONTRACT_JSON_PATH)
    parser.add_argument("--preflight-path", default=DEFAULT_C2B_PREFLIGHT_JSON_PATH)
    parser.add_argument("--dry-run-path", default=DEFAULT_C2B_DRY_RUN_JSON_PATH)
    parser.add_argument("--json-report-path", default=DEFAULT_C2B_JSON_REPORT_PATH)
    parser.add_argument("--markdown-report-path", default=DEFAULT_C2B_MD_REPORT_PATH)
    parser.add_argument("--rollback-sql-path", default=DEFAULT_C2B_ROLLBACK_SQL_PATH)
    parser.add_argument("--c2b-run-id", required=True)
...
def format_summary(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "closed signal enrichment execute",
