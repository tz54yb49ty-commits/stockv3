#!/usr/bin/env python3
"""Run N1 condition source activation 20260605 preflight or final execute."""
...
from ashare_v3.ingestion.condition_source_activation_20260605_execute import (  # noqa: E402
    DEFAULT_PATHS,
    TDX_ROOT,
    TRADE_DATE,
    ConditionSourceActivation20260605Blocked,
    DefaultConditionSourceActivation20260605SourceBuilder,
    build_commit_plan,
...
def build_default_source_builder(**kwargs) -> DefaultConditionSourceActivation20260605SourceBuilder:
    args = kwargs.get("args")
    return DefaultConditionSourceActivation20260605SourceBuilder(
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--execute-report-json", default=str(DEFAULT_PATHS["execute_report_json"]))
    parser.add_argument("--execute-report-md", default=str(DEFAULT_PATHS["execute_report_md"]))
...
def write_execute_report_files(report: dict, *, json_path: str | Path, markdown_path: str | Path) -> None:
    json_file = Path(json_path)
    markdown_file = Path(markdown_path)
    json_file.parent.mkdir(parents=True, exist_ok=True)
    markdown_file.parent.mkdir(parents=True, exist_ok=True)
    json_file.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    markdown_file.write_text(render_execute_report_markdown(report), encoding="utf-8")


def render_execute_report_markdown(report: dict) -> str:
    return f"""# N1 Condition Source 20260605 Activation Execute Report
...
```json
{json.dumps((report.get("commit_result") or {}).get("row_counts") or {}, ensure_ascii=False, indent=2)}
```

Rollback SQL: `{((report.get("commit_result") or {}).get("rollback_sql_path"))}`
"""
...
def main(argv: list[str] | None = None, *, dependencies: dict | None = None) -> int:
    ...
    if args.execute:
        ...
        except ConditionSourceActivation20260605Blocked as exc:
    ...
    execute_report = {
        "stage": "N1 condition source activation 20260605 execute",
        "layer_role": "N1_ingestion",
        "result": "EXECUTE_PASS",
        "source_batch_id": commit_result.get("batch_id"),
    ...
    print(json.dumps(execute_report, ensure_ascii=False, indent=2))
    return 0
