#!/usr/bin/env python3
"""Run N1 condition source activation 20260526 preflight or final execute.

The execute path is wired but guarded by explicit flags. Without all flags this
script refreshes preflight/contract artifacts and exits before source row build
or PostgreSQL commit.
"""
...
from ashare_v3.ingestion.condition_source_activation_20260526_execute import (  # noqa: E402
    ConditionSourceActivation20260526Blocked,
    DefaultConditionSourceActivation20260526SourceBuilder,
    build_commit_plan,
...
def build_default_source_builder(**kwargs) -> DefaultConditionSourceActivation20260526SourceBuilder:
    args = kwargs.get("args")
    return DefaultConditionSourceActivation20260526SourceBuilder(
...
def main(argv: list[str] | None = None, *, dependencies: dict | None = None) -> int:
    ...
    if args.execute:
        ...
        except ConditionSourceActivation20260526Blocked as exc:
    ...
    print(
        json.dumps(
            {
                "stage": "N1 condition source activation 20260526 execute",
