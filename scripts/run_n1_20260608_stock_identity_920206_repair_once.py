#!/usr/bin/env python3
"""Run N1 scoped stock_identity repair for 920206.BJ on 20260608."""
...
from ashare_v3.ingestion.stock_identity_920206_repair_20260608 import (  # noqa: E402
    DEFAULT_PATHS,
    ...
    TRADE_DATE,
    DefaultStockIdentity920206Repair20260608SourceAdapter,
...
def build_default_source_adapter(**kwargs) -> DefaultStockIdentity920206Repair20260608SourceAdapter:
    return DefaultStockIdentity920206Repair20260608SourceAdapter(token=load_tushare_token())


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--trade-date", default=TRADE_DATE)
    parser.add_argument("--identity-key", default=EXPECTED_IDENTITY_KEY)
    parser.add_argument("--ts-code", default=EXPECTED_TS_CODE)
    ...
    parser.add_argument("--implementation-json", default=str(IMPLEMENTATION_REPORT_JSON))
    parser.add_argument("--implementation-md", default=str(IMPLEMENTATION_REPORT_MD))
...
def main(argv: list[str] | None = None, *, dependencies: dict | None = None) -> int:
    ...
    try:
        validate_target_request(trade_date=args.trade_date, identity_key=args.identity_key, ts_code=args.ts_code)
        if args.execute:
            validate_execute_request(
    ...
    try:
        adapter = deps["source_adapter_factory"](args=args)
        evidence = adapter.fetch_source_evidence(trade_date=args.trade_date, ts_code=args.ts_code)
        source_report = validate_source_evidence(evidence)
    except StockIdentity920206Repair20260608Blocked as exc:
        source_report = blocked_source_report(str(exc))

    contract = build_execute_contract(snapshot, source_report=source_report)
    ...
    if not args.no_write_report:
        ...
        write_implementation_files(implementation, json_path=args.implementation_json, markdown_path=args.implementation_md)
    ...
    try:
        if evidence is None:
            raise StockIdentity920206Repair20260608Blocked("source evidence was not loaded")
        rows = build_target_identity_rows(evidence)
        validation_report = validate_source_rows(rows)
        validate_commit_preconditions(snapshot=snapshot, validation_report=validation_report)
    ...
    print(
        json.dumps(
            {
                "stage": "N1 stock_identity 920206 20260608 repair execute",
