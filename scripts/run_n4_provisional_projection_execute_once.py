#!/usr/bin/env python3
"""Run one N4 provisional B2 projection execute invocation."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run N4 provisional B2 projection execute once.")
    ...
    parser.add_argument("--source-projection-run-id", required=True)
    ...
    parser.add_argument("--no-previous-baseline", action="store_true")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = run_provisional_projection_once(
            ...
            no_previous_baseline=args.no_previous_baseline,
            previous_trigger_run_id=args.previous_trigger_run_id,
        )
    except ProvisionalProjectionExecuteBlocked as exc:
