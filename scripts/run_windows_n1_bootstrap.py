...
def main() -> int:
    ...
    if args.source_trade_date and not args.daily:
        parser.error("--source-trade-date requires --daily")
    today = date.today()
    source_trade_date = resolve_daily_source_trade_date(
        args.source_trade_date, today=today,
    )
    config = (
        WindowsN1BootstrapConfig(
            artifact_root=Path(args.artifact_root),
            start_date=source_trade_date,
    ...
    if args.plan:
        ...
        "source_trade_date": source_trade_date if args.daily else None,
    ...
    if args.daily:
        now = datetime.now()
        if (
            daily_cutoff_is_required(source_trade_date, today=today)
            and not at_or_after_daily_cutoff(now)
        ):
            print(json.dumps({
            ...
            return 0
        if not calendar_date_is_open(calendar, source_trade_date):
            print(json.dumps({
    ...
    with psycopg.connect(PASSWORDLESS_APP_DSN, connect_timeout=8) as connection:
        ...
        if args.schema_only:
            ...
            return 0
        if args.daily:
            existing_status = repository.fastlane_completion_status(source_trade_date)
            if existing_status == "passed":
                print(json.dumps({
                    "result": "N1_DAILY_ALREADY_COMPLETE",
                    "trade_date": source_trade_date,
                    "database_written": False,
                    "market_request_count": 0,
                }, ensure_ascii=False))
                return 0
        failure_run_id = "windows_n1_failed_" + datetime.now(timezone.utc).strftime(
            "%Y%m%dT%H%M%S%fZ"
        )
        try:
            ...
            source_counts_before = (
                repository.daily_bar_counts(source_trade_date) if args.daily else None
            )
            with TdxClient(timeout=8) as eltdx_client:
                ...
                result = execute_bootstrap(
                    config=config, stage_handlers=production.handlers()
                )
            if not result.n1_data_ready:
                raise RuntimeError("N1 bootstrap did not reach n1_data_ready")
            if args.daily:
                source_counts_after = repository.daily_bar_counts(source_trade_date)
                completion_details = {
                    "today_counts_before": source_counts_before,
                    "today_counts_after": source_counts_after,
            ...
            if args.daily:
                ...
                try:
                    repository.mark_fastlane_failed(
                        trade_date=source_trade_date,
                    ...
                    raise RuntimeError(
                        f"{error_summary}; failure marker write failed: {marker_exc}"
                    ) from exc
            raise
