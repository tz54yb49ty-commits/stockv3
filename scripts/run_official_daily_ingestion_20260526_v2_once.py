#!/usr/bin/env python3
"""Run N1 official daily 20260526 v2 ingestion preflight or final execute.

The execute path is wired, but guarded by four explicit flags. Without all
flags this script only refreshes preflight artifacts and exits before source
fetch or commit.
"""
...
from ashare_v3.ingestion.official_daily_20260526_v2_execute import (  # noqa: E402
    DEFAULT_PATHS,
    TRADE_DATE,
...
def build_default_source_adapter(**kwargs) -> DefaultOfficialDaily20260526V2SourceAdapter:
    args = kwargs.get("args")
    return DefaultOfficialDaily20260526V2SourceAdapter(
...
def main(argv: list[str] | None = None, *, dependencies: dict | None = None) -> int:
    ...
    if args.execute:
        ...
        except OfficialDaily20260526V2ExecuteBlocked as exc:
    ...
    print(
        json.dumps(
            {
                "stage": "N1 official daily 20260526 v2 ingestion execute",
