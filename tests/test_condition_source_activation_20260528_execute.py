...
from ashare_v3.ingestion.condition_source_activation_20260528_execute import (
    ...
    SOURCE_VERSIONS,
    TRADE_DATE,
    ConditionSourceActivation20260528Blocked,
    build_commit_plan,
...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "run_condition_source_activation_20260528_once.py"


def load_runner_module():
    spec = importlib.util.spec_from_file_location("n1_condition_source_activation_20260528_runner", SCRIPT_PATH)
...
class ConditionSourceActivation20260528ExecuteTests(unittest.TestCase):
    def test_missing_required_flags_block(self) -> None:
        ...
        for execute, confirmed, commit, message in cases:
            with self.subTest(message=message):
                with self.assertRaisesRegex(ConditionSourceActivation20260528Blocked, message):
                    validate_execute_request(
    ...
    def test_official_no_trade_rows_inserted_blocks_validation(self) -> None:
        bundle = valid_source_bundle()
        no_trade_identity = OFFICIAL_NO_TRADE_IDENTITIES[0]
        bundle["stock_daily_basic"][0] = dict(bundle["stock_daily_basic"][0], stock_identity_key=no_trade_identity)
        bundle["stock_financial"][0] = dict(bundle["stock_financial"][0], stock_identity_key=no_trade_identity)
    ...
    def test_20260528_no_trade_manifest_uses_corrected_members(self) -> None:
    ...
    def test_commit_preconditions_block_p0_validation(self) -> None:
        ...
        with self.assertRaisesRegex(ConditionSourceActivation20260528Blocked, "stock_daily_basic_row_count_mismatch"):
            validate_commit_preconditions(
                snapshot=sample_pass_snapshot(),
    ...
    def test_rollback_sql_exists(self) -> None:
        self.assertTrue((PROJECT_ROOT / "sql" / "N1_condition_source_20260528_activation_rollback.sql").exists())
