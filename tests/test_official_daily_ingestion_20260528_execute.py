...
from ashare_v3.ingestion.official_daily_20260528_execute import (
    ...
    STALE_IDENTITY_KEY,
    TRADE_DATE,
    OfficialDaily20260528ExecuteBlocked,
    build_commit_plan,
...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "run_official_daily_ingestion_20260528_once.py"


def load_runner_module():
    spec = importlib.util.spec_from_file_location("n1_official_daily_20260528_runner", SCRIPT_PATH)
...
class RunnerHarness:
    def __init__(self, *, snapshot: dict | None = None, bundle: dict | None = None) -> None:
        ...
        self.adapter = Fake20260528Adapter(self.bundle)
    ...
    def source_adapter_factory(self, **kwargs) -> Fake20260528Adapter:
...
class OfficialDaily20260528ExecuteTests(unittest.TestCase):
    def test_manifest_correction_replaces_300561_with_001331(self) -> None:
        self.assertIn("stock:SZ:001331", OFFICIAL_NO_TRADE_IDENTITIES)
        self.assertNotIn("stock:SZ:300561", OFFICIAL_NO_TRADE_IDENTITIES)
        self.assertEqual(OFFICIAL_NO_TRADE_CORRECTION_EVIDENCE["added_to_no_trade_manifest"]["suspend_type"], "S")
        self.assertEqual(OFFICIAL_NO_TRADE_CORRECTION_EVIDENCE["removed_from_no_trade_manifest"]["suspend_type"], "R")
    ...
    def test_missing_each_final_flag_blocks(self) -> None:
        ...
        for execute, confirmed, source_fetch, commit, message in cases:
            with self.subTest(message=message):
                with self.assertRaisesRegex(OfficialDaily20260528ExecuteBlocked, message):
                    validate_execute_request(
    ...
    def test_preflight_pass_ready_for_final_gate(self) -> None:
        ...
        evidence = report["source_gap_summary"]["official_no_trade_correction_evidence"]
        self.assertEqual(evidence["added_to_no_trade_manifest"]["identity_key"], "stock:SZ:001331")
        self.assertEqual(evidence["removed_from_no_trade_manifest"]["identity_key"], "stock:SZ:300561")

    def test_success_commit_plan_has_5506_83_428(self) -> None:
        scope = expected_scope()
        source_bundle = fetch_official_daily_sources(
            adapter=Fake20260528Adapter(valid_bundle(scope)),
            trade_date=TRADE_DATE,
        ...
        self.assertEqual(
            plan["manifest"]["official_no_trade_correction_evidence"]["removed_from_no_trade_manifest"]["identity_key"],
            "stock:SZ:300561",
        )
    ...
    def test_rollback_sql_path_exists(self) -> None:
        self.assertTrue((PROJECT_ROOT / "sql" / "N1_official_daily_20260528_ingestion_rollback.sql").exists())
