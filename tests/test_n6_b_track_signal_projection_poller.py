...
class N6BTrackSignalProjectionPollerTest(unittest.TestCase):
    ...
    def test_projection_writer_freezes_typed_trade_date_and_slim_list_payload(self):
        ...
        row = {
            ...
            "source_payload_json": {
                ...
                "payload_json": {
                    ...
                    "trigger_price": "12.34",
                    "triggered_periods": ["D"],
                    "source_trigger_event_id": "trigger-1",
                },
            },
            "trace_json": {"source_n4_run_id": "n4-run-1"},
        ...
        fields = poller.build_projection_read_model_fields(row, event)

        self.assertEqual(fields["for_trade_date"].isoformat(), "2026-07-06")
        self.assertEqual(fields["list_payload_version"], "n6_projection_list_v1")
        self.assertEqual(fields["list_payload_json"]["primary_trigger_period"], "D")
        self.assertEqual(fields["list_payload_json"]["triggered_periods"], ["D"])
        self.assertEqual(fields["list_payload_json"]["n4_trigger_event_id"], "trigger-1")
        self.assertEqual(fields["list_payload_json"]["score"], "88.5")

        nested_fields = poller.build_projection_read_model_fields(
            {
                "source_payload_json": {
                    "payload_json": {
                        "trace_json": {
                ...
                "trace_json": {"source_n4_run_id": "n4-run-kept-separate"},
                "display_payload_json": {},
            },
            event,
        )
        self.assertEqual(nested_fields["list_payload_json"]["primary_trigger_period"], "30m")
        self.assertEqual(nested_fields["list_payload_json"]["triggered_periods"], ["30m"])

    def test_projection_insert_includes_read_model_columns(self):
        import run_n6_b_track_signal_projection_poller_once as poller
        ...
        class Cursor:
            ...
            def fetchone(self):
                return {"user_signal_projection_id": 41}

        columns = (
            "user_projection_run_id", "user_id", "user_filter_profile_id", "user_watchlist_id",
            "permission_scope", "source_layer", "source_event_id", "source_outbox_id",
            "source_event_type", "source_event_schema_version", "source_event_dedup_key",
            "source_action_event_id", "source_action_event_type", "source_action_run_id", "action_state",
            "action_mark", "condition_key", "original_condition_key", "trace_json", "projection_policy",
            "asset_kind", "identity_key", "code", "name", "direction", "signal_type", "target_price",
            "current_price", "expected_return_pct", "board_identity_key", "board_code", "board_name",
            "source_display_table", "source_condition_display_basis_id", "source_condition_display_run_id",
            "projection_status", "source_payload_json", "display_payload_json", "for_trade_date",
            "list_payload_version", "list_payload_json",
        )
        row = {column: None for column in columns}
        row.update(
            {
                "for_trade_date": datetime(2026, 7, 6).date(),
                "list_payload_version": "n6_projection_list_v1",
                "list_payload_json": {},
            }
        )
        cursor = Cursor()

        projection_id = poller.insert_signal_projection(cursor, row)

        self.assertEqual(projection_id, 41)
        normalized = " ".join(cursor.sql.split())
        self.assertIn("for_trade_date, list_payload_version, list_payload_json", normalized)
        self.assertEqual(len(cursor.params), 41)
