...
from ashare_v3.ingestion.official_daily_20260529_execute import (
    ...
    STALE_IDENTITY_KEY,
    TRADE_DATE,
    OfficialDaily20260529ExecuteBlocked,
    build_commit_plan,
...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "run_official_daily_ingestion_20260529_once.py"


def load_runner_module():
    spec = importlib.util.spec_from_file_location("n1_official_daily_20260529_runner", SCRIPT_PATH)
...
class Fake20260529Adapter:
    def __init__(self, bundle: dict) -> None:
        self.bundle = bundle
        self.called = False
        self.stock_daily_source_count = 5506
        self.stock_adj_factor_source_count = EXPECTED_STOCK_ADJ_FACTOR_ROWS
        self.stock_matched_identity_count = 5506
...
class RunnerHarness:
    def __init__(self, *, snapshot: dict | None = None, bundle: dict | None = None) -> None:
        ...
        self.adapter = Fake20260529Adapter(self.bundle)
    ...
    def source_adapter_factory(self, **kwargs) -> Fake20260529Adapter:
...
class OfficialDaily20260529ExecuteTests(unittest.TestCase):
    def test_manifest_has_20260529_no_trade_set(self) -> None:
        self.assertEqual(len(OFFICIAL_NO_TRADE_IDENTITIES), 19)
        self.assertIn("stock:SZ:000691", OFFICIAL_NO_TRADE_IDENTITIES)
        self.assertIn("stock:SZ:001331", OFFICIAL_NO_TRADE_IDENTITIES)
        self.assertNotIn("stock:SZ:300561", OFFICIAL_NO_TRADE_IDENTITIES)
    ...
    def test_missing_each_final_flag_blocks(self) -> None:
        ...
        for execute, confirmed, source_fetch, commit, message in cases:
            with self.subTest(message=message):
                with self.assertRaisesRegex(OfficialDaily20260529ExecuteBlocked, message):
                    validate_execute_request(
    ...
    def test_preflight_pass_ready_for_final_gate(self) -> None:
        ...
        self.assertFalse(report["source_fetch"]["will_call_external_sources_this_preflight"])
        self.assertEqual(report["source_gap_summary"]["official_no_trade_manifest_rows"], 19)

    def test_success_commit_plan_has_5506_83_428(self) -> None:
        scope = expected_scope()
        source_bundle = fetch_official_daily_sources(
            adapter=Fake20260529Adapter(valid_bundle(scope)),
            trade_date=TRADE_DATE,
        ...
        self.assertEqual(validation["quality"]["p1_count"], 20)
        self.assertEqual(validation["source_probe_counts"]["stock_adj_factor_rows"], 5525)
        self.assertEqual(validation["source_probe_counts"]["matched_identity_rows"], 5506)
        self.assertEqual(validation["source_probe_counts"]["unmapped_tushare_daily_rows"], 0)
        no_trade_item = next(item for item in validation["quality_items"] if item["gate_name"] == "official_no_trade_manifest")
        self.assertIn("stock:SZ:000691", no_trade_item["details"]["identity_keys"])
        self.assertIn("stock:SZ:001331", no_trade_item["details"]["identity_keys"])
        self.assertNotIn("stock:SZ:300561", no_trade_item["details"]["identity_keys"])
        self.assertEqual(plan["row_counts"], {"stock": 5506, "index": 83, "board": 428, "total": 6017})
        self.assertEqual(len(plan["manifest"]["official_no_trade"]), 19)
    ...
    def test_rollback_sql_path_exists(self) -> None:
        self.assertTrue((PROJECT_ROOT / "sql" / "N1_official_daily_20260529_ingestion_rollback.sql").exists())
    ...
    def test_cli_missing_source_fetch_flag_blocks_before_source_fetch(self) -> None:
        runner = load_runner_module()
        harness = RunnerHarness()
