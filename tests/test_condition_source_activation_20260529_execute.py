...
from ashare_v3.ingestion.condition_source_activation_20260529_execute import (
    ...
    SOURCE_VERSIONS,
    TRADE_DATE,
    ConditionSourceActivation20260529Blocked,
    build_commit_plan,
...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "run_condition_source_activation_20260529_once.py"


def load_runner_module():
    spec = importlib.util.spec_from_file_location("n1_condition_source_activation_20260529_runner", SCRIPT_PATH)
...
class ConditionSourceActivation20260529ExecuteTests(unittest.TestCase):
    def test_missing_required_flags_block(self) -> None:
        ...
        for execute, confirmed, commit, message in cases:
            with self.subTest(message=message):
                with self.assertRaisesRegex(ConditionSourceActivation20260529Blocked, message):
                    validate_execute_request(
    ...
    def test_preflight_ready_for_final_gate(self) -> None:
        ...
        self.assertIn("20260529", report["execute_command_template"])
    ...
    def test_success_commit_plan_has_expected_counts_and_manifest_quality(self) -> None:
        ...
        self.assertEqual(len(plan["manifests"]["official_no_trade_manifest"]), 19)
        self.assertEqual(len(plan["manifests"]["stale_identity_manifest"]), 1)
        self.assertEqual(plan["active_source_version_rows"][0]["activated_by"], "n1_condition_source_activation_20260529_execute_runner")

    def test_20260529_no_trade_manifest_includes_new_zero_volume_name(self) -> None:
        manifest_keys = {row["identity_key"] for row in official_no_trade_manifest()}

        self.assertIn("stock:SZ:000691", manifest_keys)
        self.assertIn("stock:SZ:001331", manifest_keys)
        self.assertNotIn("stock:SZ:300561", manifest_keys)
        self.assertEqual(len(manifest_keys), 19)

    def test_commit_writes_allowed_tables_only_and_uses_20260529_source(self) -> None:
        bundle = valid_source_bundle()
        validation = validate_source_bundle(bundle=bundle, snapshot=sample_pass_snapshot())
        plan = build_commit_plan(bundle=bundle, validation_report=validation, baseline=sample_pass_snapshot())
        ...
        self.assertIn("n1.condition_source_activation.20260529.v1", joined)
        self.assertNotIn("n1.condition_source_activation.20260528.v1", joined)
        status_updates = [
            params
    ...
    def test_commit_preconditions_block_p0_validation(self) -> None:
        ...
        with self.assertRaisesRegex(ConditionSourceActivation20260529Blocked, "stock_daily_basic_row_count_mismatch"):
            validate_commit_preconditions(
    ...
    def test_rollback_sql_exists(self) -> None:
        self.assertTrue((PROJECT_ROOT / "sql" / "N1_condition_source_20260529_activation_rollback.sql").exists())
