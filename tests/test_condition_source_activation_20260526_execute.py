...
from ashare_v3.ingestion.condition_source_activation_20260526_execute import (
    ConditionSourceActivation20260526Blocked,
    build_commit_plan,
...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "run_condition_source_activation_20260526_once.py"


def load_runner_module():
    spec = importlib.util.spec_from_file_location("n1_condition_source_activation_execute_runner", SCRIPT_PATH)
...
class ConditionSourceActivation20260526ExecuteTests(unittest.TestCase):
    def test_missing_required_flags_block(self) -> None:
        ...
        for execute, confirmed, commit, message in cases:
            with self.subTest(message=message):
                with self.assertRaisesRegex(ConditionSourceActivation20260526Blocked, message):
                    validate_execute_request(
    ...
    def test_success_commit_plan_has_expected_counts(self) -> None:
        bundle = valid_source_bundle()
        validation = validate_source_bundle(bundle=bundle, snapshot=sample_pass_snapshot())
        ...
        self.assertEqual(validation["quality"], {"p0_count": 0, "p1_count": 0, "p2_count": 1})
        self.assertEqual(
            plan["row_counts"],
            {
    ...
    def test_commit_preconditions_block_p0_validation(self) -> None:
        ...
        with self.assertRaisesRegex(ConditionSourceActivation20260526Blocked, "stock_daily_basic_row_count_mismatch"):
            validate_commit_preconditions(
                snapshot=sample_pass_snapshot(),
    ...
    def test_rollback_sql_exists(self) -> None:
        self.assertTrue((PROJECT_ROOT / "sql" / "N1_condition_source_20260526_activation_rollback.sql").exists())
