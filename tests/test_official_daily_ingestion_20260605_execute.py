...
from ashare_v3.ingestion.official_daily_20260605_execute import (
    ACTIVE_STOCK_IDENTITY_SCOPE_KEY,
    ACTIVE_STOCK_IDENTITY_SOURCE_BATCH_ID,
    ACTIVE_STOCK_IDENTITY_SOURCE_VERSION,
    ...
    SOURCE_VERSIONS,
    TRADE_DATE,
    OfficialDaily20260605ExecuteBlocked,
    build_commit_plan,
...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "run_official_daily_ingestion_20260605_once.py"


def load_runner_module():
    spec = importlib.util.spec_from_file_location("n1_official_daily_20260605_runner", SCRIPT_PATH)
...
class RunnerHarness:
    def __init__(self) -> None:
        ...
        self.adapter = Fake20260605Adapter(valid_bundle(self.scope))
    ...
    def source_adapter_factory(self, **kwargs) -> Fake20260605Adapter:
...
class OfficialDaily20260605ExecuteTests(unittest.TestCase):
    def test_constants_match_20260605_contract(self) -> None:
        self.assertEqual("20260605", TRADE_DATE)
        self.assertEqual("A_STOCK:20260605", ACTIVE_STOCK_IDENTITY_SCOPE_KEY)
        self.assertEqual("stock_identity_20260605_v1", ACTIVE_STOCK_IDENTITY_SOURCE_VERSION)
        self.assertEqual("stock_identity_refresh_20260605_920211_v1", ACTIVE_STOCK_IDENTITY_SOURCE_BATCH_ID)
        self.assertEqual(
            {"stock_daily_bar_fact": 5514, "index_daily_bar_fact": 83, "board_daily_bar_fact": 428, "total_daily_fact": 6025},
            EXPECTED_ROWS,
        )
        self.assertEqual(12, len(OFFICIAL_NO_TRADE_IDENTITIES))
    ...
    def test_missing_each_final_flag_blocks(self) -> None:
        ...
        for execute, confirmed, source_fetch, commit, message in cases:
            with self.subTest(message=message):
                with self.assertRaisesRegex(OfficialDaily20260605ExecuteBlocked, message):
                    validate_execute_request(
    ...
    def test_sample_stock_source_probe_blocks_on_missing_920211_identity(self) -> None:
        preflight = build_execute_preflight_report(
            snapshot=sample_pass_snapshot(),
            stock_probe=sample_stock_source_probe(),
            index_board_probe={"result": "FULL_PROBE_PASS"},
        ...
        self.assertEqual("PREFLIGHT_BLOCKED", preflight["result"])
        self.assertIn("stock_identity_refresh_required", preflight["production_execute_blockers"])
        self.assertEqual(["920211.BJ"], preflight["stock_source_probe"]["stock_source"]["unmapped_sample"])

    def test_preflight_blocks_stale_active_stock_identity_scope(self) -> None:
        snapshot = sample_pass_snapshot()
        snapshot["active_stock_identity_scope"] = {
            "scope_key": "A_STOCK:20260529",
        ...
        preflight = build_execute_preflight_report(
            snapshot=snapshot,
            stock_probe={"result": "STOCK_PROBE_PASS", "stock_source": {"unmapped_count": 0}},
            index_board_probe={"result": "FULL_PROBE_PASS"},
            execute_requested=False,
            user_confirmed=False,
            source_fetch_enabled=False,
        ...
        self.assertEqual("PREFLIGHT_BLOCKED", preflight["result"])
        self.assertIn("active_stock_identity_scope_missing", preflight["production_execute_blockers"])

    def test_success_commit_plan_has_expected_rows(self) -> None:
        scope = expected_scope()
        bundle = fetch_official_daily_sources(
            adapter=Fake20260605Adapter(valid_bundle(scope)),
            trade_date=TRADE_DATE,
        ...
        self.assertEqual({"stock": 5514, "index": 83, "board": 428, "total": 6025}, plan["row_counts"])
        self.assertEqual(12, len(plan["manifest"]["official_no_trade"]))

    def test_missing_000638_no_trade_source_row_is_corrected_before_validation(self) -> None:
        scope = expected_scope()
        source_bundle = valid_bundle(scope)
        source_bundle["official_no_trade_manifest"] = [
            row for row in source_bundle["official_no_trade_manifest"] if row["identity_key"] != "stock:SZ:000638"
        ]

        fetched = fetch_official_daily_sources(
            adapter=Fake20260605Adapter(source_bundle),
            trade_date=TRADE_DATE,
            expected_scope=scope,
            source_fetch_enabled=True,
        )
        validation = validate_source_bundle(bundle=fetched, expected_scope=scope, trade_date=TRADE_DATE)

        corrected = [
            row for row in fetched["official_no_trade_manifest"] if row["identity_key"] == "stock:SZ:000638"
        ]
        self.assertEqual(1, len(corrected))
        self.assertIs(False, corrected[0]["writes_stock_daily_bar_fact"])
        self.assertEqual(12, len(fetched["official_no_trade_manifest"]))
        self.assertEqual("VALIDATION_PASS", validation["result"])
        self.assertNotIn("official_no_trade_manifest_mismatch", validation["blockers"])
