...
from ashare_v3.ingestion.condition_source_activation_20260605_execute import (
    ALLOWED_FUTURE_WRITE_TABLES,
    BATCH_ID,
    EXPECTED_REFERENCE_ROWS,
    SOURCE_VERSIONS,
    TRADE_DATE,
    ConditionSourceActivation20260605Blocked,
    build_commit_plan,
...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "run_condition_source_activation_20260605_once.py"


def load_runner_module():
    spec = importlib.util.spec_from_file_location("n1_condition_source_activation_20260605_runner", SCRIPT_PATH)
...
class ConditionSourceActivation20260605ExecuteTests(unittest.TestCase):
    def test_missing_required_flags_block(self) -> None:
        ...
        for execute, confirmed, commit, message in cases:
            with self.subTest(message=message):
                with self.assertRaisesRegex(ConditionSourceActivation20260605Blocked, message):
                    validate_execute_request(
    ...
    def test_preflight_ready_for_final_gate(self) -> None:
        ...
        self.assertEqual(report["quality"], {"p0_count": 0, "p1_count": 3, "p2_count": 1})
        self.assertIn("20260605", report["execute_command_template"])

    def test_expected_rows_match_official_daily_universe_and_current_tdx_membership(self) -> None:
        self.assertEqual(
            build_expected_rows(sample_pass_snapshot()),
            {
    ...
    def test_success_commit_plan_has_expected_counts_and_manifest_quality(self) -> None:
        ...
        self.assertEqual(validation["quality"], {"p0_count": 0, "p1_count": 3, "p2_count": 1})
        self.assertEqual(plan["row_counts"]["total"], 80831)
        self.assertEqual(len(plan["manifests"]["official_no_trade_manifest"]), 12)
        self.assertEqual(len(plan["manifests"]["stale_identity_manifest"]), 1)
        self.assertEqual(plan["active_source_version_rows"][0]["activated_by"], "n1_condition_source_activation_20260605_execute_runner")

    def test_20260605_no_trade_manifest_matches_official_daily_post_review(self) -> None:
        manifest_keys = {row["identity_key"] for row in official_no_trade_manifest()}

        self.assertEqual(len(manifest_keys), 12)
        self.assertIn("stock:SZ:000638", manifest_keys)
        self.assertIn("stock:BJ:920305", manifest_keys)
        self.assertIn("stock:SH:688121", manifest_keys)
        self.assertNotIn("stock:SZ:000668", manifest_keys)
        self.assertNotIn("stock:SZ:000736", manifest_keys)
        self.assertNotIn("stock:SZ:002200", manifest_keys)
        self.assertNotIn("stock:SZ:300175", manifest_keys)
        self.assertNotIn("stock:SZ:300685", manifest_keys)

    def test_commit_writes_allowed_tables_only_and_uses_20260605_source(self) -> None:
        bundle = valid_source_bundle()
        validation = validate_source_bundle(bundle=bundle, snapshot=sample_pass_snapshot())
        plan = build_commit_plan(bundle=bundle, validation_report=validation, baseline=sample_pass_snapshot())
        ...
        self.assertIn("n1.condition_source_activation.20260605.v1", joined)
        self.assertNotIn("n1.condition_source_activation.20260602.v1", joined)
        status_updates = [
            params
    ...
    def test_rollback_sql_exists(self) -> None:
        self.assertTrue((PROJECT_ROOT / "sql" / "N1_condition_source_20260605_activation_rollback.sql").exists())
    ...
    def test_cli_execute_writes_json_and_markdown_report(self) -> None:
        runner = load_runner_module()
        harness = RunnerHarness()
        stdout = io.StringIO()
        stderr = io.StringIO()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            json_path = Path(tmpdir) / "execute_report.json"
            md_path = Path(tmpdir) / "execute_report.md"
            with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
                result = runner.main(
                    [
                        "--execute",
            ...
            self.assertEqual(report["source_batch_id"], BATCH_ID)
            self.assertEqual(report["commit_result"]["row_counts"]["total"], 80831)
            self.assertEqual(report["source_validation"]["result"], "VALIDATION_PASS")
            self.assertFalse(report["side_effects"]["enters_n2_n3_n4_n5_n6"])
            markdown = md_path.read_text()
            self.assertIn("N1 Condition Source 20260605 Activation Execute Report", markdown)
            self.assertIn("EXECUTE_PASS", markdown)
