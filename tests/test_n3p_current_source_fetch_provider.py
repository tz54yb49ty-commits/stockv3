...
class N3PCurrentSourceFetchProviderBackendTest(unittest.TestCase):
    ...
    def test_n3p_trigger_proof_preflight_provider_materializes_writer_artifacts(self) -> None:
        ...
        from ashare_v3.market.artifact_blob import canonical_json_bytes, read_artifact_blob
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            refs = contract["materialized_source_payload_refs"]
            self.assertEqual(set(refs), {"candidates", "n4_context_snapshot_rows", "previous_day_cumulative_rows"})
            for field, reference in refs.items():
                self.assertEqual(reference["schema"], "artifact_blob_v1")
                self.assertEqual(read_artifact_blob(reference, base_path=contract_path.parent), [
                    {"identity_key": "stock:SH:600000"}
                ] if field != "previous_day_cumulative_rows" else [
                    {"identity_key": "stock:SH:600000", "previous_day_elapsed_amount": "123.45"}
                ])
            self.assertNotIn("candidates", contract["materialized_source_payload_overlay"])
            legacy_contract = dict(contract)
            legacy_overlay = dict(contract["materialized_source_payload_overlay"])
            legacy_overlay.update({
                field: read_artifact_blob(reference, base_path=contract_path.parent)
                for field, reference in refs.items()
            })
            legacy_contract["materialized_source_payload_overlay"] = legacy_overlay
            legacy_contract.pop("materialized_source_payload_refs")
            legacy_contract.pop("materialized_source_payload_refs_base_path")
            source_payload = {"stock_quote_rows": [{"identity_key": "stock:SH:600000"}]}
            v2_rows = writer.materialize_source_payload_from_contract(contract, source_payload)
            v1_rows = writer.materialize_source_payload_from_contract(legacy_contract, source_payload)
            self.assertEqual(v2_rows, v1_rows)
            self.assertEqual(
                hashlib.sha256(canonical_json_bytes(v2_rows)).hexdigest(),
                hashlib.sha256(canonical_json_bytes(v1_rows)).hexdigest(),
            )
        ...
        self.assertIn("materialized_source_payload_refs", contract)
