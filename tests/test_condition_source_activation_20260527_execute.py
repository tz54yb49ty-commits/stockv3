...
from ashare_v3.ingestion.condition_source_activation_20260527_execute import (
    ...
    SOURCE_VERSIONS,
    TRADE_DATE,
    ConditionSourceActivation20260527Blocked,
    build_commit_plan,
...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "run_condition_source_activation_20260527_once.py"


def load_runner_module():
    spec = importlib.util.spec_from_file_location("n1_condition_source_activation_20260527_runner", SCRIPT_PATH)
...
class ConditionSourceActivation20260527ExecuteTests(unittest.TestCase):
    def test_missing_required_flags_block(self) -> None:
        ...
        for execute, confirmed, commit, message in cases:
            with self.subTest(message=message):
                with self.assertRaisesRegex(ConditionSourceActivation20260527Blocked, message):
                    validate_execute_request(
    ...
    def test_commit_preconditions_block_p0_validation(self) -> None:
        ...
        with self.assertRaisesRegex(ConditionSourceActivation20260527Blocked, "stock_daily_basic_row_count_mismatch"):
            validate_commit_preconditions(
                snapshot=sample_pass_snapshot(),
    ...
    def test_rollback_sql_exists(self) -> None:
        self.assertTrue((PROJECT_ROOT / "sql" / "N1_condition_source_20260527_activation_rollback.sql").exists())
