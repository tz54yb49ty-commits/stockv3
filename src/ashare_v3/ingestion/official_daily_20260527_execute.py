"""N1 official daily 20260527 execute runner support.

This module wires the 20260527 official daily execute path behind explicit
...
TRADE_DATE = "20260527"
EXPECTED_PREV_TRADE_DATE = "20260526"
EXPECTED_NEXT_TRADE_DATE = "20260528"
BATCH_ID = "official_daily_ingest_20260527_v1"
CONTRACT_SOURCE_VERSION = BATCH_ID
SOURCE_VERSIONS = {
    "stock": "stock_daily_20260527_v1",
    "index": "index_daily_20260527_v1",
    "board": "board_daily_20260527_v1",
}
ACTIVE_DATA_TYPES = {
    "stock": "stock_daily",
    "index": "index_daily",
...
DEFAULT_PATHS = {
    "contract_json": Path("docs/N1_official_daily_20260527_execute_contract.json"),
    "contract_md": Path("docs/N1_OFFICIAL_DAILY_20260527_EXECUTE_CONTRACT.md"),
    "preflight_json": Path("docs/N1_official_daily_20260527_execute_preflight.json"),
    "preflight_md": Path("docs/N1_OFFICIAL_DAILY_20260527_EXECUTE_PREFLIGHT.md"),
    "rollback_sql": Path("sql/N1_official_daily_20260527_rollback.sql"),
}
BOARD_881_RE = re.compile(r"^881\d{3}$")


class OfficialDaily20260527ExecuteBlocked(RuntimeError):
    """Raised when the 20260527 official daily execute gate is blocked."""
...
def validate_execute_request(*, execute_requested: bool, user_confirmed: bool, postgres_commit_enabled: bool) -> None:
    if not execute_requested:
        raise OfficialDaily20260527ExecuteBlocked("missing --execute")
    if not user_confirmed:
        raise OfficialDaily20260527ExecuteBlocked("missing --user-confirmed")
    if not postgres_commit_enabled:
        raise OfficialDaily20260527ExecuteBlocked("missing --postgres-commit-enabled")


class DefaultOfficialDaily20260527SourceAdapter:
    ...
    def _pro(self) -> Any:
        if self._tushare_client is None:
            if not self.tushare_token:
                raise OfficialDaily20260527ExecuteBlocked("TUSHARE_TOKEN is required for official daily fetch")
...
def fetch_official_daily_sources(*, adapter: Any, trade_date: str, expected_scope: Mapping[str, Any]) -> dict[str, Any]:
...
def validate_commit_preconditions(
    *,
    ...
    if blockers:
        raise OfficialDaily20260527ExecuteBlocked(", ".join(sorted(dict.fromkeys(blockers))))
...
def build_commit_plan(
    *,
    ...
    if int(validation_report.get("p0_count") or 0) != 0:
        raise OfficialDaily20260527ExecuteBlocked("source validation P0 must be zero before commit plan")
    rows_by_asset = normalize_bundle_rows(bundle)
    quality_rows = [
        {
            "source_batch_id": BATCH_ID,
    ...
    quality_rows.append(
        {
            ...
            "data_domain": "common",
            "data_type": "official_daily",
            "gate_name": "official_daily_20260527_manifest_details",
            "severity": "P1",
            "status": "warning",
    ...
    active_rows = [
        {
            ...
            "activated_by": "n1_official_daily_20260527_execute_runner",
        }
        for asset in ("stock", "index", "board")
    ]
    return normalize_jsonable(
...
def execute_commit_transaction(
    ...
    *,
    ...
    if unexpected_tables:
        raise OfficialDaily20260527ExecuteBlocked(f"unexpected write tables: {unexpected_tables}")
...
def build_execute_preflight_report(
    ...
    *,
    ...
    return normalize_jsonable(
        {
            "stage": "N1 official daily 20260527 ingestion execute preflight",
            ...
            "execute_pipeline": {
                "wired": True,
                "enabled_for_this_run": bool(execute_requested and user_confirmed and postgres_commit_enabled),
                "sequence": [
                    "load_contract",
                    "refresh_db_baseline",
            ...
            "execute_command_template": (
                "PYTHONPATH=src python3 scripts/run_official_daily_ingestion_20260527_once.py "
                "--trade-date 20260527 --execute --user-confirmed --postgres-commit-enabled"
            ),
...
def build_execute_contract(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return normalize_jsonable(
        {
            "stage": "N1 official daily 20260527 ingestion execute contract",
            "layer_role": "N1_ingestion",
            ...
            "manifests": {
                ...
                "index_canonical_mapping": dict(CANONICAL_IDENTITY_MAPPING),
                "new_identity_included": ["stock:SH:688635", "stock:BJ:920161"],
            },
            "quality_gate": {
...
def build_snapshot_from_db(*, dsn: str, trade_date: str = TRADE_DATE) -> dict[str, Any]:
    if trade_date != TRADE_DATE:
        raise ValueError("This runner is fixed to trade_date=20260527")
...
def fetch_index_expansion_scope(cur: Any, *, trade_date: str) -> list[dict[str, Any]]:
    ...
    for source_key, canonical_key in CANONICAL_IDENTITY_MAPPING.items():
        if source_key in rows_by_key:
            ...
            if not canonical:
                raise OfficialDaily20260527ExecuteBlocked(f"missing canonical identity for {source_key} -> {canonical_key}")
...
def resolve_membership_trade_date(cur: Any, *, trade_date: str) -> str:
    ...
    if not value:
        raise OfficialDaily20260527ExecuteBlocked("index_membership_fact scope missing")
    return str(value)
...
def validate_execute_contract(contract: Mapping[str, Any]) -> None:
    if contract.get("result") != "DESIGN_PASS":
        raise OfficialDaily20260527ExecuteBlocked("execute_contract_not_design_pass")
    if (contract.get("source_batch_id") or contract.get("contract_batch_id")) != BATCH_ID:
        raise OfficialDaily20260527ExecuteBlocked("execute_contract_batch_id_mismatch")
    if dict(contract.get("source_versions") or {}) != dict(SOURCE_VERSIONS):
        raise OfficialDaily20260527ExecuteBlocked("execute_contract_source_versions_mismatch")
...
def render_preflight_markdown(report: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# N1 Official Daily 20260527 Ingestion Execute Preflight",
            "",
            "layer_role: `N1_ingestion`",
...
def render_contract_markdown(contract: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            "# N1 Official Daily 20260527 Ingestion Execute Contract",
            "",
            "layer_role: `N1_ingestion`",
...
def build_preflight_quality(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    items = [
        quality_item(
            ...
            expected="row=1,is_open=true,prev=20260526,next=20260528",
        ...
        quality_item(
            ...
            expected="A_STOCK:20260527 -> stock_identity_20260527_v1",
        ...
        quality_item(
            ...
            expected=18,
            actual=18,
            details={"writes_stock_daily_bar_fact": False, "identity_keys": list(OFFICIAL_NO_TRADE_IDENTITIES)},
        ),
...
def active_stock_identity_ready(snapshot: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and str(scope.get("scope_key") or "") == "A_STOCK:20260527"
        and str(scope.get("source_version") or "") == "stock_identity_20260527_v1"
    )
...
def insert_ingest_batch(cur: Any, commit_plan: Mapping[str, Any]) -> None:
    cur.execute(
        ...
        VALUES (
          ...
          'n1.official_daily.20260527.source_fetch', %(source_version)s,
