"""Execute runner support for N1 official daily ingestion 20260605.

This module binds the verified official-daily execute mechanics to the
20260605 source scope. Importing or running preflight remains read-only; writes
...
TRADE_DATE = "20260605"
EXPECTED_PREV_TRADE_DATE = "20260604"
EXPECTED_NEXT_TRADE_DATE = "20260608"
...
BATCH_ID = "official_daily_ingest_20260605_v1"
...
EXPECTED_STOCK_ADJ_FACTOR_ROWS = 5526
...
OFFICIAL_NO_TRADE_CORRECTION_EVIDENCE: dict[str, Any] = {
    "stock:SZ:000638": {
        ...
        "reason": "absent from official daily source and not inserted as bar; Tushare suspend manifest did not return this identity for 20260605",
        "source": "N1_official_daily_20260605_execute_blocked_report",
        "blocked_report": "docs/N1_official_daily_20260605_ingestion_execute_blocked_report.json",
    }
}
ALLOWED_FUTURE_WRITE_TABLES = template.ALLOWED_FUTURE_WRITE_TABLES
DEFAULT_PATHS = {
    "dry_run_json": Path("docs/N1_official_daily_20260605_ingestion_dry_run_report.json"),
    "dry_run_md": Path("docs/N1_OFFICIAL_DAILY_20260605_INGESTION_DRY_RUN_REPORT.md"),
    "contract_json": Path("docs/N1_official_daily_20260605_ingestion_execute_contract.json"),
    "contract_md": Path("docs/N1_OFFICIAL_DAILY_20260605_INGESTION_EXECUTE_CONTRACT.md"),
    "preflight_json": Path("docs/N1_official_daily_20260605_ingestion_execute_preflight.json"),
    "preflight_md": Path("docs/N1_OFFICIAL_DAILY_20260605_INGESTION_EXECUTE_PREFLIGHT.md"),
    "rollback_sql": Path("sql/N1_official_daily_20260605_ingestion_rollback.sql"),
    "stock_probe_json": Path("docs/N1_official_daily_20260605_stock_source_probe.json"),
    "index_board_probe_json": Path("docs/N1_official_daily_20260605_index_board_source_probe.json"),
    "index_board_probe_md": Path("docs/N1_OFFICIAL_DAILY_20260605_INDEX_BOARD_SOURCE_PROBE.md"),
}


class OfficialDaily20260605ExecuteBlocked(RuntimeError):
    """Raised when the guarded 20260605 execute path refuses to continue."""


class DefaultOfficialDaily20260605SourceAdapter(template.DefaultOfficialDaily20260601SourceAdapter):
    """20260605 adapter using the verified official daily source routes."""
...
def _convert_blocker(exc: Exception) -> OfficialDaily20260605ExecuteBlocked:
    return OfficialDaily20260605ExecuteBlocked(str(exc))
...
def _replace_template_display_text(value: Any, *, parent_key: str | None = None) -> Any:
    if isinstance(value, str):
        if parent_key in _TEMPLATE_DISPLAY_TEXT_KEYS:
            return value.replace("20260601", "20260605").replace("20260529", "20260605")
        return value
...
def active_stock_identity_ready(snapshot: Mapping[str, Any]) -> bool:
    scope = dict(snapshot.get("active_stock_identity_scope") or {})
    return (
        ...
        and str(scope.get("source_batch_id") or "") == ACTIVE_STOCK_IDENTITY_SOURCE_BATCH_ID
    )


def _active_stock_identity_quality_item(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    scope = dict(snapshot.get("active_stock_identity_scope") or {})
    ...
    return {
        ...
        "actual": scope,
        "details": {
            "blocking_execute": status != "passed",
            "lineage_guard": "official_daily_20260605_active_stock_identity",
        },
    }


def _with_active_stock_identity_guard(report: Mapping[str, Any], snapshot: Mapping[str, Any]) -> dict[str, Any]:
    fixed = dict(report)
    quality = dict(fixed.get("quality") or {})
    items = [dict(item) for item in quality.get("items") or []]
    if not any(item.get("gate_name") == "active_stock_identity_scope_ready" for item in items):
        items.append(_active_stock_identity_quality_item(snapshot))
    quality.update(_quality_counts(items))
    quality["items"] = items
    fixed["quality"] = quality
    fixed.setdefault("baseline", {})["active_stock_identity_scope"] = dict(snapshot.get("active_stock_identity_scope") or {})
    if not active_stock_identity_ready(snapshot):
        blockers = list(fixed.get("production_execute_blockers") or [])
        if "active_stock_identity_scope_missing" not in blockers:
            blockers.append("active_stock_identity_scope_missing")
        fixed["production_execute_blockers"] = blockers
        fixed["result"] = "PREFLIGHT_BLOCKED"
        fixed["blocked"] = True
        fixed["final_execute_gate_allowed"] = False
        fixed["production_execute_allowed"] = False
    return fixed
...
def _fix_markdown_template_title(path: str | Path) -> None:
    ...
    text = text.replace("# N1 Official Daily 20260601", "# N1 Official Daily 20260605")
    text = text.replace("# N1 Official Daily 20260529", "# N1 Official Daily 20260605")
...
def sample_pass_snapshot() -> dict[str, Any]:
    ...
    snapshot["stock_active_universe"] = STOCK_SCOPE_BREAKDOWN["stock_identity_active_universe"]
    snapshot["next_trade_calendar_detail"] = {"trade_date": EXPECTED_NEXT_TRADE_DATE, "row_count": 1}
    return snapshot


def sample_stock_source_probe() -> dict[str, Any]:
    return {
        "result": "STOCK_PROBE_BLOCKED",
        "stock_source": {
            "tushare_daily_count": EXPECTED_ROWS["stock_daily_bar_fact"],
            "adj_factor_count": EXPECTED_STOCK_ADJ_FACTOR_ROWS,
            "matched_identity_count": 5513,
            "unmapped_count": 1,
            "unmapped_sample": ["920211.BJ"],
            "unmapped_reason": "stock_identity_refresh_required",
            "adj_minus_daily_active_identity_count": len(OFFICIAL_NO_TRADE_IDENTITIES),
            "duplicate_daily_ts_code_count": 0,
            "stock_identity_refresh_required": True,
            "post_identity_refresh_expected_matched_identity_count": EXPECTED_MATCHED_STOCK_IDENTITY_ROWS,
            "post_identity_refresh_expected_unmapped_count": EXPECTED_UNMAPPED_TUSHARE_DAILY_ROWS,
        },
        "quality": {
            "p0_count": 1,
            "p1_count": 0,
            "p2_count": 0,
            "p0_items": ["stock_identity_refresh_required"],
            "p1_items": [],
            "p2_items": [],
        },
    }
...
def _with_20260605_no_trade_corrections(bundle: Mapping[str, Any]) -> dict[str, Any]:
    fixed = json.loads(json.dumps(bundle, ensure_ascii=False, default=str))
    manifest = [dict(row) for row in fixed.get("official_no_trade_manifest") or []]
    manifest_ids = {str(row.get("identity_key") or "") for row in manifest}
    stock_row_ids = {str(row.get("identity_key") or "") for row in fixed.get("stock") or []}
    for identity_key, evidence in OFFICIAL_NO_TRADE_CORRECTION_EVIDENCE.items():
        if identity_key in manifest_ids or identity_key in stock_row_ids:
            continue
        ...
        manifest.append(
            {
                ...
                "source_proof_json": {
                    ...
                    "official_daily_source_row_present": False,
...
def fetch_official_daily_sources(
    *,
    ...
    with patched_template():
        try:
            bundle = template.fetch_official_daily_sources(
                adapter=adapter,
                trade_date=trade_date,
            ...
            raise _convert_blocker(exc) from exc
    return _with_20260605_no_trade_corrections(bundle)
...
def validate_commit_preconditions(
    *,
    ...
    if not active_stock_identity_ready(snapshot):
        raise OfficialDaily20260605ExecuteBlocked("active_stock_identity_scope_missing")
    with patched_template():
        try:
...
def build_dry_run_report(*, snapshot: Mapping[str, Any], stock_probe: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return _with_next_calendar_warning(_with_active_stock_identity_guard(report, snapshot), snapshot)
...
def build_execute_contract(*, snapshot: Mapping[str, Any], stock_probe: Mapping[str, Any]) -> dict[str, Any]:
    ...
    stock_source = dict(stock_probe.get("stock_source") or {})
    fixed.setdefault("baseline", {})["active_stock_identity_scope"] = dict(snapshot.get("active_stock_identity_scope") or {})
    fixed.setdefault("source_probe_requirements", {})["active_stock_identity_scope"] = (
        f"{ACTIVE_STOCK_IDENTITY_SCOPE_KEY} -> {ACTIVE_STOCK_IDENTITY_SOURCE_VERSION}"
    )
    if stock_source.get("stock_identity_refresh_required"):
        fixed.setdefault("source_probe_requirements", {})["stock"] = (
            "BLOCKED: stock_identity_refresh_required for 920211.BJ before production commit"
        )
    ...
    if stock_source.get("stock_identity_refresh_required"):
        fixed["implementation_status"]["runner_readiness"] = "blocked_by_stock_identity_refresh_required"
        fixed["implementation_status"]["next_required_step"] = "N1 stock_identity refresh for 920211.BJ"
    return fixed
...
def build_execute_preflight_report(
    *,
    ...
    fixed = _with_next_calendar_warning(_with_active_stock_identity_guard(_fix_report_text(report), snapshot), snapshot)
    fixed["stock_source_probe"] = dict(stock_probe)
    stock_source = dict(stock_probe.get("stock_source") or {})
    if stock_source.get("stock_identity_refresh_required"):
        quality = dict(fixed.get("quality") or {})
        items = [dict(item) for item in quality.get("items") or []]
        if not any(item.get("gate_name") == "stock_identity_refresh_required" for item in items):
            items.append(
                {
                    "gate_name": "stock_identity_refresh_required",
                    "severity": "P0",
                    "status": "failed",
                    "expected": "all Tushare daily rows have stock_identity",
                    "actual": {
                        "unmapped_count": int(stock_source.get("unmapped_count") or 0),
                        "unmapped_sample": list(stock_source.get("unmapped_sample") or []),
                    },
                    "details": {
                        "blocking_execute": True,
                        "recommended_next_gate": "N1 stock_identity refresh before official daily 20260605 execute",
                    },
                }
            )
        quality.update(_quality_counts(items))
        ...
        fixed["quality"] = quality
        blockers = list(fixed.get("production_execute_blockers") or [])
        if "stock_identity_refresh_required" not in blockers:
            blockers.append("stock_identity_refresh_required")
        fixed["production_execute_blockers"] = blockers
        fixed["result"] = "PREFLIGHT_BLOCKED"
        fixed["blocked"] = True
        fixed["final_execute_gate_allowed"] = False
    ...
    fixed["execute_runner"]["runner_readiness"] = fixed["runner_readiness"]
    fixed["execute_runner"]["final_execute_gate_allowed"] = fixed.get("final_execute_gate_allowed") is True
    return fixed
