"""N1 official daily 20260528 execute runner support.

This module wires the 20260528 official daily execute path behind explicit
...
EXPECTED_NEXT_TRADE_DATE = "20260529"
ACTIVE_STOCK_IDENTITY_SCOPE_KEY = "A_STOCK:20260527"
ACTIVE_STOCK_IDENTITY_SOURCE_VERSION = "stock_identity_20260527_v1"
ACTIVE_STOCK_IDENTITY_SOURCE_BATCH_ID = "stock_identity_refresh_20260527_v1"
ACTIVE_STOCK_IDENTITY_PREVIOUS_SOURCE_VERSION = "stock_identity_20260522_v1"
BATCH_ID = "official_daily_ingest_20260528_v1"
CONTRACT_SOURCE_VERSION = BATCH_ID
SOURCE_VERSIONS = {
    "stock": "stock_daily_20260528_v1",
    "index": "index_daily_20260528_v1",
    "board": "board_daily_20260528_v1",
}
ACTIVE_DATA_TYPES = {
    "stock": "stock_daily",
    "index": "index_daily",
...
STOCK_SCOPE_BREAKDOWN = {
    "stock_identity_active_universe": 5525,
    "stale_identity_excluded": 1,
    "effective_universe_excluding_stale": 5524,
...
OFFICIAL_NO_TRADE_CORRECTION_EVIDENCE = {
    "added_to_no_trade_manifest": {
        "identity_key": "stock:SZ:001331",
        "ts_code": "001331.SZ",
        "name": "胜通能源",
        "tushare_daily_rows": 0,
        "adj_factor_rows": 1,
        "suspend_d_rows": 1,
        "suspend_type": "S",
        "bak_daily_rows": 1,
        "bak_daily_zero_volume": True,
        "disposition": "official_no_trade_manifest_only",
    },
    "removed_from_no_trade_manifest": {
        "identity_key": "stock:SZ:300561",
        "ts_code": "300561.SZ",
        "name": "汇金科技",
        "tushare_daily_rows": 1,
        "adj_factor_rows": 1,
        "suspend_d_rows": 1,
        "suspend_type": "R",
        "bak_daily_rows": 1,
        "bak_daily_has_volume": True,
        "disposition": "include_in_stock_daily_bar_fact",
    },
}
...
DEFAULT_PATHS = {
    "dry_run_json": Path("docs/N1_official_daily_20260528_ingestion_dry_run_report.json"),
    "dry_run_md": Path("docs/N1_OFFICIAL_DAILY_20260528_INGESTION_DRY_RUN_REPORT.md"),
    "contract_json": Path("docs/N1_official_daily_20260528_ingestion_execute_contract.json"),
    "contract_md": Path("docs/N1_OFFICIAL_DAILY_20260528_INGESTION_EXECUTE_CONTRACT.md"),
    "preflight_json": Path("docs/N1_official_daily_20260528_ingestion_execute_preflight.json"),
    "preflight_md": Path("docs/N1_OFFICIAL_DAILY_20260528_INGESTION_EXECUTE_PREFLIGHT.md"),
    "rollback_sql": Path("sql/N1_official_daily_20260528_ingestion_rollback.sql"),
}
BOARD_881_RE = re.compile(r"^881\d{3}$")


class OfficialDaily20260528ExecuteBlocked(RuntimeError):
    """Raised when the 20260528 official daily execute gate is blocked."""
...
def validate_execute_request(
    *,
    ...
    if not execute_requested:
        raise OfficialDaily20260528ExecuteBlocked("missing --execute")
    if not user_confirmed:
        raise OfficialDaily20260528ExecuteBlocked("missing --user-confirmed")
    if not source_fetch_enabled:
        raise OfficialDaily20260528ExecuteBlocked("missing --source-fetch-enabled")
    if not postgres_commit_enabled:
        raise OfficialDaily20260528ExecuteBlocked("missing --postgres-commit-enabled")


class DefaultOfficialDaily20260528SourceAdapter:
    ...
    def _pro(self) -> Any:
        if self._tushare_client is None:
            if not self.tushare_token:
                raise OfficialDaily20260528ExecuteBlocked("TUSHARE_TOKEN is required for official daily fetch")
...
def fetch_official_daily_sources(
    *,
    ...
    if not source_fetch_enabled:
        raise OfficialDaily20260528ExecuteBlocked("source_fetch_disabled")
...
def validate_source_bundle(*, bundle: Mapping[str, Any], expected_scope: Mapping[str, Any], trade_date: str) -> dict[str, Any]:
    ...
    quality_items.append(
        quality_item(
            ...
            expected={"manifest_rows": 18, "inserted_bar_rows": 0},
...
def validate_commit_preconditions(
    *,
    ...
    if blockers:
        raise OfficialDaily20260528ExecuteBlocked(", ".join(sorted(dict.fromkeys(blockers))))
...
def build_commit_plan(
    *,
    ...
    if int(validation_report.get("p0_count") or 0) != 0:
        raise OfficialDaily20260528ExecuteBlocked("source validation P0 must be zero before commit plan")
    rows_by_asset = normalize_bundle_rows(bundle)
    quality_rows = [
        {
            "source_batch_id": BATCH_ID,
    ...
    quality_rows.append(
        {
            ...
            "data_domain": "common",
            "data_type": "official_daily",
            "gate_name": "official_daily_20260528_manifest_details",
            "severity": "P1",
            "status": "warning",
    ...
    active_rows = [
        {
            ...
            "activated_by": "n1_official_daily_20260528_execute_runner",
        }
        for asset in ("stock", "index", "board")
    ]
    return normalize_jsonable(
...
def execute_commit_transaction(
    ...
    *,
    ...
    if unexpected_tables:
        raise OfficialDaily20260528ExecuteBlocked(f"unexpected write tables: {unexpected_tables}")
...
def build_execute_preflight_report(
    ...
    *,
    ...
    return normalize_jsonable(
        {
            "stage": "N1 official daily 20260528 ingestion execute preflight",
            ...
            "execute_command_template": (
                "PYTHONPATH=src python3 scripts/run_official_daily_ingestion_20260528_once.py "
                "--trade-date 20260528 --execute --user-confirmed --source-fetch-enabled --postgres-commit-enabled"
            ),
...
def build_execute_contract(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return normalize_jsonable(
        {
            "stage": "N1 official daily 20260528 ingestion execute contract",
            "layer_role": "N1_ingestion",
...
def build_dry_run_report(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return normalize_jsonable(
        {
            "stage": "N1 official daily 20260528 ingestion dry-run",
            "layer_role": "N1_ingestion",
...
def build_snapshot_from_db(*, dsn: str, trade_date: str = TRADE_DATE) -> dict[str, Any]:
    if trade_date != TRADE_DATE:
        raise ValueError("This runner is fixed to trade_date=20260528")
...
def fetch_index_expansion_scope(cur: Any, *, trade_date: str) -> list[dict[str, Any]]:
    ...
    for source_key, canonical_key in CANONICAL_IDENTITY_MAPPING.items():
        if source_key in rows_by_key:
            ...
            if not canonical:
                raise OfficialDaily20260528ExecuteBlocked(f"missing canonical identity for {source_key} -> {canonical_key}")
...
def resolve_membership_trade_date(cur: Any, *, trade_date: str) -> str:
    ...
    if not value:
        raise OfficialDaily20260528ExecuteBlocked("index_membership_fact scope missing")
    return str(value)
...
def validate_execute_contract(contract: Mapping[str, Any]) -> None:
    if contract.get("result") != "DESIGN_PASS":
        raise OfficialDaily20260528ExecuteBlocked("execute_contract_not_design_pass")
    if (contract.get("source_batch_id") or contract.get("contract_batch_id")) != BATCH_ID:
        raise OfficialDaily20260528ExecuteBlocked("execute_contract_batch_id_mismatch")
    if dict(contract.get("source_versions") or {}) != dict(SOURCE_VERSIONS):
        raise OfficialDaily20260528ExecuteBlocked("execute_contract_source_versions_mismatch")
...
def render_dry_run_markdown(report: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# N1 Official Daily 20260528 Ingestion Dry-Run Report",
            "",
            "layer_role: `N1_ingestion`",
...
def render_preflight_markdown(report: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# N1 Official Daily 20260528 Ingestion Execute Preflight",
            "",
            "layer_role: `N1_ingestion`",
            f"状态: `{report.get('result')}`",
            "",
...
def render_contract_markdown(contract: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            "# N1 Official Daily 20260528 Ingestion Execute Contract",
            "",
            "layer_role: `N1_ingestion`",
...
def fetch_active_stock_identity_scope(cur: Any) -> dict[str, Any]:
    cur.execute(
        ...
        WHERE data_domain = 'stock'
          ...
          AND scope_key <= 'A_STOCK:20260528'
...
def build_preflight_quality(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    items = [
        quality_item(
            ...
            expected="row=1,is_open=true,prev=20260527,next=20260529",
...
def insert_ingest_batch(cur: Any, commit_plan: Mapping[str, Any]) -> None:
    cur.execute(
        ...
        VALUES (
          ...
          'n1.official_daily.20260528.source_fetch', %(source_version)s,
