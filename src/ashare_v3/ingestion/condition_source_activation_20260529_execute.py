"""Execute runner support for N1 condition source activation 20260529.

The implementation reuses the verified 20260528 activation mechanics while
binding every contract, manifest, source version, and commit marker to the
20260529 condition-source activation scope.
"""
...
from ashare_v3.ingestion import condition_source_activation_20260528_execute as template


TRADE_DATE = "20260529"
BATCH_ID = "condition_source_activation_20260529_v1"
...
ACTIVE_SCOPES = {
    "stock_daily_basic": "20260529",
    "stock_financial": "20260529",
    "index_membership": "TDX:20260529",
    "board_membership": "TDX:20260529",
}
DATA_DOMAINS = {
...
DEFAULT_PATHS = {
    "dry_run_json": Path("docs/N1_condition_source_20260529_activation_dry_run_report.json"),
    "dry_run_md": Path("docs/N1_CONDITION_SOURCE_20260529_ACTIVATION_DRY_RUN_REPORT.md"),
    "contract_json": Path("docs/N1_condition_source_20260529_activation_execute_contract.json"),
    "contract_md": Path("docs/N1_CONDITION_SOURCE_20260529_ACTIVATION_EXECUTE_CONTRACT.md"),
    "preflight_json": Path("docs/N1_condition_source_20260529_activation_execute_preflight.json"),
    "preflight_md": Path("docs/N1_CONDITION_SOURCE_20260529_ACTIVATION_EXECUTE_PREFLIGHT.md"),
    "rollback_sql": Path("sql/N1_condition_source_20260529_activation_rollback.sql"),
}

ConditionSourceActivation20260529Blocked = template.ConditionSourceActivation20260528Blocked
...
def _replace_20260528_text(value: Any) -> Any:
    if isinstance(value, str):
        return value.replace("20260528", "20260529")
    if isinstance(value, list):
        return [_replace_20260528_text(item) for item in value]
    if isinstance(value, tuple):
        return tuple(_replace_20260528_text(item) for item in value)
    if isinstance(value, dict):
        return {key: _replace_20260528_text(item) for key, item in value.items()}
    return value


def _fix_report_text(report: Mapping[str, Any]) -> dict[str, Any]:
    fixed = dict(report)
    fixed["stage"] = str(fixed.get("stage") or "").replace("20260528", "20260529")
    if "execute_command_template" in fixed:
        fixed["execute_command_template"] = str(fixed["execute_command_template"]).replace("20260528", "20260529")
    if "quality_items" in fixed:
        fixed["quality_items"] = _replace_20260528_text(fixed["quality_items"])
    return template.base.normalize_jsonable(fixed)
...
def sample_pass_snapshot() -> dict[str, Any]:
    ...
    snapshot["upstream_daily"] = {
        "stock_daily": {"active_source_version": "stock_daily_20260529_v1", "row_count": 5506},
        "index_daily": {"active_source_version": "index_daily_20260529_v1", "row_count": 83},
        "board_daily": {"active_source_version": "board_daily_20260529_v1", "row_count": 428},
    }
    ...
    return template.base.normalize_jsonable(snapshot)
...
def build_quality_items(snapshot: Mapping[str, Any]) -> list[dict[str, Any]]:
    with patched_template():
        return _replace_20260528_text(template.build_quality_items(snapshot))
...
class DefaultConditionSourceActivation20260529SourceBuilder:
    def __init__(self, *, tdx_root: str | Path = TDX_ROOT, tushare_token: str | None = None) -> None:
        self._builder = template.DefaultConditionSourceActivation20260528SourceBuilder(
            tdx_root=tdx_root,
    ...
    def build_source_bundle(self, *, dsn: str, trade_date: str, snapshot: Mapping[str, Any]) -> dict[str, Any]:
        with patched_template():
            return template.base.normalize_jsonable(
...
def build_commit_plan(
    *,
    ...
    for row in plan.get("active_source_version_rows") or []:
        row["activated_by"] = "n1_condition_source_activation_20260529_execute_runner"
    plan["manifests"] = {
        **(plan.get("manifests") or {}),
        "official_no_trade_manifest": official_no_trade_manifest(),
        "stale_identity_manifest": list(STALE_IDENTITY_MANIFEST),
    }
    return template.base.normalize_jsonable(plan)
...
def execute_commit_transaction(
    ...
    *,
    ...
    if unexpected_tables:
        raise ConditionSourceActivation20260529Blocked(f"unexpected write tables: {unexpected_tables}")
    cur = conn.cursor()
    try:
        insert_ingest_batch(cur, commit_plan)
        template.base.insert_stock_daily_basic_rows(cur, (commit_plan.get("rows") or {}).get("stock_daily_basic") or [])
        template.base.insert_stock_financial_rows(cur, (commit_plan.get("rows") or {}).get("stock_financial") or [])
        template.base.insert_index_membership_rows(cur, (commit_plan.get("rows") or {}).get("index_membership") or [])
        template.base.insert_board_membership_rows(cur, (commit_plan.get("rows") or {}).get("board_membership") or [])
        template.base.insert_quality_rows(cur, commit_plan.get("quality_rows") or [])
        template.base.insert_active_source_version_rows(cur, commit_plan.get("active_source_version_rows") or [])
...
def insert_ingest_batch(cur: Any, commit_plan: Mapping[str, Any]) -> None:
    cur.execute(
        ...
        VALUES (
          ...
          'n1.condition_source_activation.20260529.v1', %(source_version)s,
        ...
        """,
        {
            ...
            "source_params": template.base.jsonb_payload(
            ...
            "quality_gate_summary": template.base.jsonb_payload(
                {
...
def render_preflight_markdown(report: Mapping[str, Any]) -> str:
    with patched_template():
        return template.render_preflight_markdown(_fix_report_text(report)).replace("20260528", "20260529")


def render_contract_markdown(contract: Mapping[str, Any]) -> str:
    with patched_template():
        return template.render_contract_markdown(_fix_report_text(contract)).replace("20260528", "20260529")


def render_dry_run_markdown(report: Mapping[str, Any]) -> str:
    with patched_template():
        return template.render_dry_run_markdown(_fix_report_text(report)).replace("20260528", "20260529")
