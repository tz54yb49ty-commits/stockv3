"""Execute runner support for N1 condition source activation 20260528.

The module reuses the already verified 20260526 v2 activation mechanics, but
binds them to the 20260528 contract, manifests, source versions, and rollback
...
TRADE_DATE = "20260528"
BATCH_ID = "condition_source_activation_20260528_v1"
...
ACTIVE_SCOPES = {
    "stock_daily_basic": "20260528",
    "stock_financial": "20260528",
    "index_membership": "TDX:20260528",
    "board_membership": "TDX:20260528",
}
DATA_DOMAINS = {
...
DEFAULT_PATHS = {
    "dry_run_json": Path("docs/N1_condition_source_20260528_activation_dry_run_report.json"),
    "dry_run_md": Path("docs/N1_CONDITION_SOURCE_20260528_ACTIVATION_DRY_RUN_REPORT.md"),
    "contract_json": Path("docs/N1_condition_source_20260528_activation_execute_contract.json"),
    "contract_md": Path("docs/N1_CONDITION_SOURCE_20260528_ACTIVATION_EXECUTE_CONTRACT.md"),
    "preflight_json": Path("docs/N1_condition_source_20260528_activation_execute_preflight.json"),
    "preflight_md": Path("docs/N1_CONDITION_SOURCE_20260528_ACTIVATION_EXECUTE_PREFLIGHT.md"),
    "rollback_sql": Path("sql/N1_condition_source_20260528_activation_rollback.sql"),
}

ConditionSourceActivation20260528Blocked = base.ConditionSourceActivation20260526V2Blocked
...
def sample_pass_snapshot() -> dict[str, Any]:
    ...
    snapshot["upstream_daily"] = {
        "stock_daily": {"active_source_version": "stock_daily_20260528_v1", "row_count": 5506},
        "index_daily": {"active_source_version": "index_daily_20260528_v1", "row_count": 83},
        "board_daily": {"active_source_version": "board_daily_20260528_v1", "row_count": 428},
    }
...
def _quality_with_20260528_manifests(items: list[dict[str, Any]], snapshot: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        if item.get("gate_name") == "condition_source_gap_manifest":
            ...
            item["expected_value"] = "0 existing 20260528 target fact rows"
...
def build_quality_items(snapshot: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return base.normalize_jsonable(_quality_with_20260528_manifests(items, snapshot))
...
def build_execute_preflight_report(
    ...
    *,
    ...
    return base.normalize_jsonable(
        {
            "stage": "N1 condition source activation 20260528 execute preflight",
            ...
            "execute_command_template": (
                "PYTHONPATH=src python3 scripts/run_condition_source_activation_20260528_once.py "
...
def build_dry_run_report(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return base.normalize_jsonable(
        {
            "stage": "N1 condition source activation 20260528 dry-run",
            "layer_role": "N1_ingestion",
            ...
            "condition_universe_summary": {
                "official_daily_stock_rows": (snapshot.get("stock_scope") or {}).get("official_daily_stock_rows"),
                "condition_stock_rows": (snapshot.get("stock_scope") or {}).get("condition_stock_rows"),
                "official_no_trade_excluded": len(official_no_trade_manifest()),
                "stale_identity_manifest_rows": len(STALE_IDENTITY_MANIFEST),
            },
            "membership_summary": snapshot.get("membership_tdx") or {},
            "baseline": {
                "upstream_daily": snapshot.get("upstream_daily") or {},
                "current_target_fact_rows": snapshot.get("current_target_fact_rows") or {},
                "active_target_source_versions": snapshot.get("active_target_source_versions") or [],
                "target_source_version_conflicts": snapshot.get("target_source_version_conflicts") or {},
                "contract_batch_exists": bool(snapshot.get("contract_batch_exists")),
...
def build_execute_contract(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return base.normalize_jsonable(
        {
            "stage": "N1 condition source activation 20260528 execute contract",
            "layer_role": "N1_ingestion",
...
class DefaultConditionSourceActivation20260528SourceBuilder:
...
def validate_source_bundle(*, bundle: Mapping[str, Any], snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    quality_items = _quality_with_20260528_manifests(list(report.get("quality_items") or []), snapshot)
    quality = summarize_quality(quality_items)
    report = dict(report)
...
def validate_commit_preconditions(
    *,
    ...
    if blockers:
        raise ConditionSourceActivation20260528Blocked(", ".join(sorted(dict.fromkeys(blockers))))
...
def build_commit_plan(
    *,
    ...
    for row in plan.get("active_source_version_rows") or []:
        row["activated_by"] = "n1_condition_source_activation_20260528_execute_runner"
    plan["manifests"] = {
...
def execute_commit_transaction(
    ...
    *,
    ...
    if unexpected_tables:
        raise ConditionSourceActivation20260528Blocked(f"unexpected write tables: {unexpected_tables}")
    cur = conn.cursor()
    try:
...
def insert_ingest_batch(cur: Any, commit_plan: Mapping[str, Any]) -> None:
    cur.execute(
        ...
        VALUES (
          ...
          'n1.condition_source_activation.20260528.v1', %(source_version)s,
...
def write_dry_run_files(report: Mapping[str, Any], *, json_path: str | Path, markdown_path: str | Path) -> None:
    ...
    Path(markdown_path).write_text(render_dry_run_markdown(report), encoding="utf-8")


def render_dry_run_markdown(report: Mapping[str, Any]) -> str:
    return f"""# N1 Condition Source 20260528 Activation Dry-Run
...
- official_no_trade excluded: `{report["condition_universe_summary"]["official_no_trade_excluded"]}`
- stale identity manifest rows: `{report["condition_universe_summary"]["stale_identity_manifest_rows"]}`
...
def render_preflight_markdown(report: Mapping[str, Any]) -> str:
    return f"""# N1 Condition Source 20260528 Activation Preflight
...
def render_contract_markdown(contract: Mapping[str, Any]) -> str:
    return f"""# N1 Condition Source 20260528 Activation Contract
