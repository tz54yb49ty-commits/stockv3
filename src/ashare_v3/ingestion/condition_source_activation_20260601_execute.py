"""Execute runner support for N1 condition source activation 20260601.
...
20260601 condition-source activation scope.
"""
...
TRADE_DATE = "20260601"
BATCH_ID = "condition_source_activation_20260601_v1"
...
ACTIVE_SCOPES = {
    "stock_daily_basic": "20260601",
    "stock_financial": "20260601",
    "index_membership": "TDX:20260601",
    "board_membership": "TDX:20260601",
}
DATA_DOMAINS = {
...
DEFAULT_PATHS = {
    "dry_run_json": Path("docs/N1_condition_source_20260601_activation_dry_run_report.json"),
    "dry_run_md": Path("docs/N1_CONDITION_SOURCE_20260601_ACTIVATION_DRY_RUN_REPORT.md"),
    "contract_json": Path("docs/N1_condition_source_20260601_activation_execute_contract.json"),
    "contract_md": Path("docs/N1_CONDITION_SOURCE_20260601_ACTIVATION_EXECUTE_CONTRACT.md"),
    "preflight_json": Path("docs/N1_condition_source_20260601_activation_execute_preflight.json"),
    "preflight_md": Path("docs/N1_CONDITION_SOURCE_20260601_ACTIVATION_EXECUTE_PREFLIGHT.md"),
    "rollback_sql": Path("sql/N1_condition_source_20260601_activation_rollback.sql"),
}

ConditionSourceActivation20260601Blocked = template.ConditionSourceActivation20260529Blocked
...
def _replace_20260529_text(value: Any) -> Any:
    if isinstance(value, str):
        return value.replace("20260529", "20260601")
...
def _fix_report_text(report: Mapping[str, Any]) -> dict[str, Any]:
    fixed = dict(report)
    fixed["stage"] = str(fixed.get("stage") or "").replace("20260529", "20260601")
    if "execute_command_template" in fixed:
        fixed["execute_command_template"] = str(fixed["execute_command_template"]).replace("20260529", "20260601")
...
def sample_pass_snapshot() -> dict[str, Any]:
    ...
    snapshot["upstream_daily"] = {
        "stock_daily": {"active_source_version": "stock_daily_20260601_v1", "row_count": 5508},
        "index_daily": {"active_source_version": "index_daily_20260601_v1", "row_count": 83},
        "board_daily": {"active_source_version": "board_daily_20260601_v1", "row_count": 428},
    }
...
class DefaultConditionSourceActivation20260601SourceBuilder:
...
def build_commit_plan(
    *,
    ...
    for row in plan.get("active_source_version_rows") or []:
        row["activated_by"] = "n1_condition_source_activation_20260601_execute_runner"
    plan["manifests"] = {
...
def execute_commit_transaction(
    ...
    *,
    ...
    if unexpected_tables:
        raise ConditionSourceActivation20260601Blocked(f"unexpected write tables: {unexpected_tables}")
    cur = conn.cursor()
    try:
...
def insert_ingest_batch(cur: Any, commit_plan: Mapping[str, Any]) -> None:
    cur.execute(
        ...
        VALUES (
          ...
          'n1.condition_source_activation.20260601.v1', %(source_version)s,
...
def render_preflight_markdown(report: Mapping[str, Any]) -> str:
    with patched_template():
        return template.render_preflight_markdown(_fix_report_text(report)).replace("20260529", "20260601")


def render_contract_markdown(contract: Mapping[str, Any]) -> str:
    with patched_template():
        return template.render_contract_markdown(_fix_report_text(contract)).replace("20260529", "20260601")


def render_dry_run_markdown(report: Mapping[str, Any]) -> str:
    with patched_template():
        return template.render_dry_run_markdown(_fix_report_text(report)).replace("20260529", "20260601")
