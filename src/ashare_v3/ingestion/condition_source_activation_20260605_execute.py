"""Execute runner support for N1 condition source activation 20260605.

The implementation reuses the verified 20260602 activation mechanics while
binding every contract, manifest, source version, and commit marker to the
20260605 condition-source activation scope.
"""
...
from ashare_v3.ingestion import condition_source_activation_20260602_execute as template

_base = template._base


TRADE_DATE = "20260605"
BATCH_ID = "condition_source_activation_20260605_v1"
...
ACTIVE_SCOPES = {
    "stock_daily_basic": "20260605",
    "stock_financial": "20260605",
    "index_membership": "TDX:20260605",
    "board_membership": "TDX:20260605",
}
DATA_DOMAINS = {
...
DEFAULT_PATHS = {
    "dry_run_json": Path("docs/N1_condition_source_20260605_activation_dry_run_report.json"),
    "dry_run_md": Path("docs/N1_CONDITION_SOURCE_20260605_ACTIVATION_DRY_RUN_REPORT.md"),
    "contract_json": Path("docs/N1_condition_source_20260605_activation_execute_contract.json"),
    "contract_md": Path("docs/N1_CONDITION_SOURCE_20260605_ACTIVATION_EXECUTE_CONTRACT.md"),
    "preflight_json": Path("docs/N1_condition_source_20260605_activation_execute_preflight.json"),
    "preflight_md": Path("docs/N1_CONDITION_SOURCE_20260605_ACTIVATION_EXECUTE_PREFLIGHT.md"),
    "execute_report_json": Path("docs/N1_condition_source_20260605_activation_execute_report.json"),
    "execute_report_md": Path("docs/N1_CONDITION_SOURCE_20260605_ACTIVATION_EXECUTE_REPORT.md"),
    "rollback_sql": Path("sql/N1_condition_source_20260605_activation_rollback.sql"),
}

ConditionSourceActivation20260605Blocked = template.ConditionSourceActivation20260602Blocked
...
def _replace_20260602_text(value: Any) -> Any:
    if isinstance(value, str):
        text = value
        for old in ("20260528", "20260529", "20260601", "20260602"):
            text = text.replace(old, "20260605")
        return text
    if isinstance(value, list):
        return [_replace_20260602_text(item) for item in value]
    if isinstance(value, tuple):
        return tuple(_replace_20260602_text(item) for item in value)
    if isinstance(value, dict):
        return {key: _replace_20260602_text(item) for key, item in value.items()}
    return value


def _fix_report_text(report: Mapping[str, Any]) -> dict[str, Any]:
    fixed = dict(report)
    fixed["stage"] = str(fixed.get("stage") or "").replace("20260602", "20260605")
    if "execute_command_template" in fixed:
        fixed["execute_command_template"] = str(fixed["execute_command_template"]).replace("20260602", "20260605")
    if "quality_items" in fixed:
        fixed["quality_items"] = _replace_20260602_text(fixed["quality_items"])
...
def sample_pass_snapshot() -> dict[str, Any]:
    ...
    snapshot["upstream_daily"] = {
        "stock_daily": {"active_source_version": "stock_daily_20260605_v1", "row_count": 5514},
        "index_daily": {"active_source_version": "index_daily_20260605_v1", "row_count": 83},
        "board_daily": {"active_source_version": "board_daily_20260605_v1", "row_count": 428},
    }
    snapshot["stock_scope"] = {
        **(snapshot.get("stock_scope") or {}),
        "active_stock_identity_rows": 5526,
        "official_daily_stock_rows": 5514,
        "condition_stock_rows": 5514,
...
def build_quality_items(snapshot: Mapping[str, Any]) -> list[dict[str, Any]]:
    with patched_template():
        return _replace_20260602_text(template.build_quality_items(snapshot))
...
def build_execute_preflight_report(
    ...
    *,
    ...
    fixed = _fix_report_text(report)
    if "execute_command_template" in fixed:
        fixed["execute_command_template"] = (
            f"{fixed['execute_command_template']} "
            f"--execute-report-json {DEFAULT_PATHS['execute_report_json']} "
            f"--execute-report-md {DEFAULT_PATHS['execute_report_md']}"
        )
    return fixed
...
class DefaultConditionSourceActivation20260605SourceBuilder:
    def __init__(self, *, tdx_root: str | Path = TDX_ROOT, tushare_token: str | None = None) -> None:
        self._builder = template.DefaultConditionSourceActivation20260602SourceBuilder(
            tdx_root=tdx_root,
...
def build_commit_plan(
    *,
    ...
    for row in plan.get("active_source_version_rows") or []:
        row["activated_by"] = "n1_condition_source_activation_20260605_execute_runner"
    plan["manifests"] = {
...
def execute_commit_transaction(
    ...
    *,
    ...
    if unexpected_tables:
        raise ConditionSourceActivation20260605Blocked(f"unexpected write tables: {unexpected_tables}")
    cur = conn.cursor()
    try:
...
def insert_ingest_batch(cur: Any, commit_plan: Mapping[str, Any]) -> None:
    cur.execute(
        ...
        VALUES (
          ...
          'n1.condition_source_activation.20260605.v1', %(source_version)s,
...
def write_preflight_files(report: Mapping[str, Any], *, json_path: str | Path, markdown_path: str | Path) -> None:
    fixed = _fix_report_text(report)
    Path(json_path).write_text(json.dumps(_base.normalize_jsonable(fixed), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    Path(markdown_path).write_text(render_preflight_markdown(fixed), encoding="utf-8")
...
def write_contract_files(contract: Mapping[str, Any], *, json_path: str | Path, markdown_path: str | Path) -> None:
    ...
    Path(json_path).write_text(json.dumps(_base.normalize_jsonable(fixed), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    Path(markdown_path).write_text(render_contract_markdown(fixed), encoding="utf-8")


def write_dry_run_files(report: Mapping[str, Any], *, json_path: str | Path, markdown_path: str | Path) -> None:
    fixed = _fix_report_text(report)
    Path(json_path).write_text(json.dumps(_base.normalize_jsonable(fixed), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    Path(markdown_path).write_text(render_dry_run_markdown(fixed), encoding="utf-8")


def render_preflight_markdown(report: Mapping[str, Any]) -> str:
    with patched_template():
        return _replace_20260602_text(template.render_preflight_markdown(_fix_report_text(report)))


def render_contract_markdown(contract: Mapping[str, Any]) -> str:
    with patched_template():
        return _replace_20260602_text(template.render_contract_markdown(_fix_report_text(contract)))


def render_dry_run_markdown(report: Mapping[str, Any]) -> str:
    with patched_template():
        return _replace_20260602_text(template.render_dry_run_markdown(_fix_report_text(report)))
