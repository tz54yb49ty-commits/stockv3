"""N1 trade-calendar patch helpers scoped to 20260529."""
...
TRADE_DATE = "20260529"
EXCHANGE = "SSE"
PATCH_BATCH_ID = "trade_calendar_20260529_patch_v1"
PATCH_SOURCE_VERSION = PATCH_BATCH_ID
PATCH_SCOPE_KEY = f"{EXCHANGE}:{TRADE_DATE}"
EXPECTED_PREV_TRADE_DATE = "20260528"
DEFAULT_FALLBACK_NEXT_TRADE_DATE = "20260601"
DEFAULT_DSN = base.DEFAULT_DSN
DEFAULT_PREFLIGHT_JSON_PATH = "docs/N1_trade_calendar_20260529_patch_preflight.json"
DEFAULT_PREFLIGHT_MARKDOWN_PATH = "docs/N1_TRADE_CALENDAR_20260529_PATCH_PREFLIGHT.md"
DEFAULT_ROLLBACK_SQL_PATH = "sql/N1_trade_calendar_20260529_patch_rollback.sql"

ALLOWED_WRITE_TABLES = base.ALLOWED_WRITE_TABLES
FORBIDDEN_WRITE_TABLES = base.FORBIDDEN_WRITE_TABLES


class CalendarPatch20260529Blocked(RuntimeError):
    """Raised when the 20260529 calendar patch safety gate is not open."""
...
def _normalize_report(report: Mapping[str, Any], snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    normalized["stage"] = "N1 trade calendar 20260529 patch preflight"
...
def validate_execute_request(
    *,
    ...
    try:
        ...
        raise CalendarPatch20260529Blocked(str(exc)) from exc
...
def build_snapshot_from_db(*, dsn: str, trade_date: str = TRADE_DATE, batch_id: str = PATCH_BATCH_ID) -> dict[str, Any]:
    with _patched_base_context():
        snapshot = base.build_snapshot_from_db(dsn=dsn, trade_date=trade_date, batch_id=batch_id)

    existing_dates = {str(row.get("trade_date") or "") for row in snapshot.get("calendar_window") or []}
    if EXPECTED_PREV_TRADE_DATE not in existing_dates:
...
def render_preflight_markdown(report: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# N1 Trade Calendar 20260529 Patch Preflight",
            "",
