"""Execute V3 20260615 C1 action-confirmation merged subscription rows.

This runner persists only the reviewed N3 subscription control rows for the
merged 806-object action-confirmation scope. It does not pull market data,
write minute facts, emit outbox events, start workers, or enter downstream
layers.
"""
...
EXPECTED_STAGE = "V3_20260615_C1_ACTION_CONFIRMATION_SCOPE_MERGED_SUBSCRIPTION_CONTROL_ROW_DRY_RUN"
EXPECTED_RUN_ID = (
...
DEFAULT_DRY_RUN_PATH = (
    "docs/V3_20260615_C1_ACTION_CONFIRMATION_SCOPE_MERGED_SUBSCRIPTION_CONTROL_ROW_DRY_RUN.json"
)
DEFAULT_JSON_REPORT_PATH = (
    "docs/V3_20260615_C1_ACTION_CONFIRMATION_SCOPE_MERGED_SUBSCRIPTION_CONTROL_ROW_EXECUTE_REPORT.json"
)
DEFAULT_MARKDOWN_REPORT_PATH = (
    "docs/V3_20260615_C1_ACTION_CONFIRMATION_SCOPE_MERGED_SUBSCRIPTION_CONTROL_ROW_EXECUTE_REPORT.md"
)
DEFAULT_ROLLBACK_SQL_PATH = "sql/V3_20260615_c1_action_confirmation_scope_merged_subscription_control_rollback.sql"
...
def validate_merged_subscription_dry_run(report: Mapping[str, Any]) -> None:
    if report.get("stage") != EXPECTED_STAGE:
        raise RuntimeError("V3 20260615 merged subscription execute blocked: dry-run stage mismatch")
    if report.get("market_data_run_id") != EXPECTED_RUN_ID:
        raise RuntimeError("V3 20260615 merged subscription execute blocked: run_id mismatch")
    if report.get("mode") != "dry_run":
        raise RuntimeError("V3 20260615 merged subscription execute blocked: dry-run mode mismatch")
    if bool(report.get("blocked")) or not bool(report.get("passed")):
        raise RuntimeError("V3 20260615 merged subscription execute blocked: dry-run did not pass")
    if int(((report.get("quality") or {}).get("p0_count")) or 0) != 0:
        raise RuntimeError("V3 20260615 merged subscription execute blocked: dry-run has P0")
    for section_name in (
        ...
        if not section.get("rows_included"):
            raise RuntimeError(f"V3 20260615 merged subscription execute blocked: {section_name} rows missing")
        if len(rows) != int(section.get("row_count") or 0):
            raise RuntimeError(f"V3 20260615 merged subscription execute blocked: {section_name} row_count mismatch")


def run_merged_subscription_execute(
    *,
    dsn: str,
    ...
    if not execute:
        raise RuntimeError("V3 20260615 merged subscription execute blocked: missing --execute")
    if not user_confirmed:
        raise RuntimeError("V3 20260615 merged subscription execute blocked: missing --user-confirmed")

    dry_run = read_json(dry_run_path)
    validate_merged_subscription_dry_run(dry_run)
    run_id = str(dry_run["market_data_run_id"])
    started_at = utc_now_iso()
    pre_backup = capture_subscription_execution_backup(
        dsn,
        phase="before_v3_20260615_c1_action_scope_merged_subscription",
        execute_run_id=run_id,
    )
    if pre_backup.get("target_run_exists"):
        raise RuntimeError(f"V3 20260615 merged subscription execute blocked: run already exists: {run_id}")
    ...
    post_backup = capture_subscription_execution_backup(
        dsn,
        phase="after_v3_20260615_c1_action_scope_merged_subscription",
        execute_run_id=run_id,
    )
    ...
    report = {
        "result": result,
        "stage": "V3_20260615_C1_ACTION_CONFIRMATION_SCOPE_MERGED_SUBSCRIPTION_CONTROL_ROW_EXECUTE",
...
def format_execute_report(report: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# V3 20260615 C1 Action-Confirmation Merged Subscription Execute Report",
            "",
