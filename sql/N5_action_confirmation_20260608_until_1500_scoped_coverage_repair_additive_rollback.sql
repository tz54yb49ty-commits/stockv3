...
-- Boundary:
--   Additive reprocess rollback: deletes only rows for this new N5 action_run
--   and this dedicated N5 consumer's inbox/checkpoint rows for the scoped N4
--   source run. Historical N5/N6 rows for the same N4 source run are preserved
--   and are not rollback blockers. It does not mutate N4 trigger facts,
--   N4 outbox status, N3 facts, user projection, voice, sim, mobile, or
--   true-trade tables.

BEGIN;

\set action_run_id 'action_consumer_execute_20260608_until_1500_scoped_coverage_repair_additive__trigger_projection_matcher_execute_20260608_v13_index_all_until_1500_formal_snapshot_fallback_retry'
\set source_trigger_run_id 'trigger_projection_matcher_execute_20260608_v13_index_all_until_1500_formal_snapshot_fallback_retry'
\set consumer_name 'n5_action_consumer_v1_until_1500_scoped_coverage_repair_additive'
...
BEGIN
  ...
  -- Additive mode: do not block on other consumers that processed the same N4 source_run_id.
  -- This rollback only removes this dedicated consumer's scoped inbox/checkpoint rows.
  ...
  LOOP
    ...
    IF v_table_regclass IS NOT NULL THEN
      ...
      INTO v_count
      USING '%' || v_action_run_id || '%', '%' || v_action_run_id || '%';
      IF v_count > 0 THEN
...
-- Additive rollback note: old same-source N5/N6 lineage intentionally remains untouched.
