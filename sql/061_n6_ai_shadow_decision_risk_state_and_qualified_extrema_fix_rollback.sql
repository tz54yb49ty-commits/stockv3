...
DECLARE
  ...
  actual_sha text;
  error_prefix text := '061_rollback_requires_fixed_state';
BEGIN
  IF SESSION_USER <> 'ashare_v3_user'
     OR CURRENT_USER <> 'ashare_v3_user' THEN
    RAISE EXCEPTION '061_rollback_owner_session_required';
  END IF;
  ...
  LOOP
    ...
    IF function_oid IS NULL THEN
      RAISE EXCEPTION '061_rollback_requires_fixed_state: %',
        expected.signature;
    END IF;
    ...
    IF actual_sha <> expected.fixed_sha THEN
      RAISE EXCEPTION '061_rollback_requires_fixed_state: body %',
        expected.signature;
    END IF;
  END LOOP;
END
...
DECLARE
  ...
  allowed_role_oid oid;
  actual_sha text;
  error_prefix text := '061_rollback_postflight_mismatch';
BEGIN
  FOR expected IN
    SELECT *
    FROM (
      VALUES
      ('n6_ai_agent_daily_summary_record(jsonb)', 'c8e0928d3afb20535792a82b20270d32d66d466a85d0d112aefc64fa8f573a5e', 'n6_ai_agent'),
      ('n6_ai_agent_context_load(text,date,integer)', 'bbcd60822e8d18e6731ac0f46e68d2dd545dea08a172f28655eead4e1444fa84', NULL::text),
      ('n6_ai_agent_proposal_create_confirm(jsonb)', '2bde0f7d24cd88bc2851fb162b8499730560e8959c8f814d6625a97fe9db063b', 'n6_ai_agent'),
      ('n6_ai_executor_risk_recheck(bigint,text)', '51faf7163dd35ead8f290c7a0ec17849f56dd0906339b8de4b7f1c5c932e834c', 'n6_virtual_executor'),
      ('n6_ai_strategy_shadow_evaluate(date,text,text)', 'a7cd3200d0c4a226c9ea03fc14e62a03f86877f768d991911eafc8b7a13c2cb2', 'n6_ai_agent'),
      ('n6_ai_agent_shadow_decision_record(jsonb)', '8bd6ed7e55ebd3f84178089e64684a66b3b2cbbf03b4f3a8115b997479b953cb', 'n6_ai_agent')
  ...
  LOOP
    ...
    IF function_oid IS NULL THEN
      RAISE EXCEPTION '061_rollback_postflight_mismatch: %', expected.signature;
    END IF;
    SELECT function_row.prosrc, function_row.prosecdef,
           function_row.proisstrict, function_row.proleakproof,
           function_row.provolatile, function_row.proparallel,
    ...
    IF actual_sha <> expected.expected_sha THEN
      RAISE EXCEPTION '061_rollback_postflight_mismatch: body %', expected.signature;
    END IF;
  END LOOP;
END
$postflight$;
