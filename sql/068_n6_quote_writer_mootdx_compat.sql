-- N6 quote writer Mootdx compatibility and confirmed manual BUY scope.
-- REVIEWED MIGRATION: execute only through a separately authorized N6 gate.
-- This migration changes no proposal, order, trade, cash, position or quote rows.
...
BEGIN
  ...
  IF scope_oid IS NULL OR helper_oid IS NULL THEN
    RAISE EXCEPTION '068_required_function_missing';
  END IF;
  ...
  IF scope_proc.owner_name <> current_user
     ...
     OR pg_catalog.encode(
        ...
        ) <> '205c61bdcabb966203eb022f61666d8c79a090ab6ac12bcf7a0e8bfb9da0fe72' THEN
    RAISE EXCEPTION '068_scope_baseline_definition_drift';
  END IF;
  IF helper_proc.owner_name <> current_user
     OR helper_proc.prosecdef IS DISTINCT FROM true
     OR helper_proc.proconfig IS DISTINCT FROM
          ARRAY['search_path=pg_catalog']::text[]
     OR pg_catalog.encode(
          pg_catalog.sha256(
            pg_catalog.convert_to(helper_proc.prosrc, 'UTF8')
          ), 'hex'
        ) <> 'a12ae3e8e8040ecb7459d08c69d263feb578b10b86d150fdb11488f6b7779d49' THEN
    RAISE EXCEPTION '068_manual_scope_helper_definition_drift';
  END IF;
  IF EXISTS (
       SELECT 1
    ...
    RAISE EXCEPTION '068_scope_baseline_acl_drift';
  END IF;
END
$preflight$;
...
BEGIN
  ...
  IF scope_proc.owner_name <> current_user
    ...
    RAISE EXCEPTION '068_postflight_definition_or_acl_drift';
  END IF;
END
$postflight$;
