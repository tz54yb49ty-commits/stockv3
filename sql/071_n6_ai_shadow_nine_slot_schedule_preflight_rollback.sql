-- Restore the exact 067 four-slot schedule function while preserving 070 and 062.
-- No function is dropped and no context, decision, observation, or trade row changes.
...
DECLARE
  ...
  actual_sha text;
  error_prefix text := '071_rollback_dependency_or_source_mismatch';
BEGIN
  IF SESSION_USER <> 'ashare_v3_user'
     OR CURRENT_USER <> 'ashare_v3_user' THEN
    RAISE EXCEPTION '071_rollback_owner_session_required';
  END IF;
...
DECLARE
  ...
  allowed_role_oid oid;
  actual_sha text;
  error_prefix text := '071_rollback_postflight_mismatch';
BEGIN
  FOR expected IN
    SELECT *
    FROM (
