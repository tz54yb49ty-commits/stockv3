...
\set today_minute_run_id 'today_minute_bar_1m_20260608_until_1500__market_data_subscription_20260608_condition_layer_20260605_to_20260608_v13_index_all_execute'
...
DECLARE
  ...
  delivered_or_delivering_outbox_refs BIGINT := 0;
  inbox_refs BIGINT := 0;
...
BEGIN
  ...
  SELECT count(*) INTO delivered_or_delivering_outbox_refs
  FROM common_event_outbox
  WHERE status IN ('delivering', 'delivered')
    AND (source_run_id = target_run_id OR payload_json::TEXT LIKE '%' || target_run_id || '%');
  ...
  IF outbox_refs <> 0
     OR delivered_or_delivering_outbox_refs <> 0
     OR inbox_refs <> 0
    ...
    RAISE EXCEPTION
      'N3-C1 rollback blocked for %, outbox=%, delivered_or_delivering=%, inbox=%, checkpoint=%, closed_30m=%, realtime_projection=%, trigger=%, trigger_state=%, action=%, n6=%, downstream_touched=%, worker=%',
      target_run_id, outbox_refs, delivered_or_delivering_outbox_refs, inbox_refs, checkpoint_refs, closed_30m_refs, realtime_projection_refs,
...
DELETE FROM common_market_data_quality_item WHERE run_id = 'today_minute_bar_1m_20260608_until_1500__market_data_subscription_20260608_condition_layer_20260605_to_20260608_v13_index_all_execute';
DELETE FROM stock_minute_bar_1m WHERE run_id = 'today_minute_bar_1m_20260608_until_1500__market_data_subscription_20260608_condition_layer_20260605_to_20260608_v13_index_all_execute' AND is_previous_day_preload = false;
DELETE FROM index_minute_bar_1m WHERE run_id = 'today_minute_bar_1m_20260608_until_1500__market_data_subscription_20260608_condition_layer_20260605_to_20260608_v13_index_all_execute' AND is_previous_day_preload = false;
DELETE FROM board_minute_bar_1m WHERE run_id = 'today_minute_bar_1m_20260608_until_1500__market_data_subscription_20260608_condition_layer_20260605_to_20260608_v13_index_all_execute' AND is_previous_day_preload = false;
DELETE FROM common_market_data_run WHERE run_id = 'today_minute_bar_1m_20260608_until_1500__market_data_subscription_20260608_condition_layer_20260605_to_20260608_v13_index_all_execute' AND downstream_layers_touched = false AND worker_started = false;
