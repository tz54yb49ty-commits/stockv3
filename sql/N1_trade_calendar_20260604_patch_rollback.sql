-- N1 trade calendar 20260604 patch rollback draft.
-- Scope: trade_calendar_20260604_patch_v1.
-- This rollback does not touch 20260603 calendar, daily fact, condition source,
-- Parquet, outbox/inbox/checkpoint, N2/N3/N4/N5/N6, worker, old system, or
-- trading state.
-- If 20260604 N1 daily facts or N2/N3/N4/N5/N6 refs exist, this rollback is
...
DO $$
DECLARE
  v_batch_id text := 'trade_calendar_20260604_patch_v1';
  v_source_version text := 'trade_calendar_20260604_patch_v1';
  v_scope_key text := 'SSE:20260604';
  v_trade_date text := '20260604';
...
BEGIN
  ...
  IF v_outbox_refs <> 0
    ...
    RAISE EXCEPTION
      'Refusing 20260604 calendar patch rollback: outbox %, inbox %, checkpoint %, N1 facts %, N2 %, N3 %, N4 %, N5 %, N6 %, missing_previous_batch %',
      v_outbox_refs,
      v_inbox_refs,
      v_checkpoint_refs,
      v_n1_fact_refs,
...
SET source_version = a.previous_source_version,
    ...
    activated_at = now(),
    activated_by = 'rollback:n1_trade_calendar_20260604_patch'
WHERE a.data_domain = 'common'
  AND a.data_type = 'trade_calendar'
  AND a.scope_key = 'SSE:20260604'
  AND a.source_batch_id = 'trade_calendar_20260604_patch_v1'
  AND a.source_version = 'trade_calendar_20260604_patch_v1'
  AND a.previous_source_version IS NOT NULL;

DELETE FROM common_active_source_version
WHERE data_domain = 'common'
  AND data_type = 'trade_calendar'
  AND scope_key = 'SSE:20260604'
  AND source_batch_id = 'trade_calendar_20260604_patch_v1'
  AND source_version = 'trade_calendar_20260604_patch_v1'
  AND previous_source_version IS NULL;

DELETE FROM common_trade_calendar
WHERE trade_date = '20260604'
  AND source_batch_id = 'trade_calendar_20260604_patch_v1'
  AND source_version = 'trade_calendar_20260604_patch_v1';

DELETE FROM common_quality_gate_result
WHERE source_batch_id = 'trade_calendar_20260604_patch_v1'
   OR source_version = 'trade_calendar_20260604_patch_v1';

DELETE FROM common_ingest_batch
WHERE batch_id = 'trade_calendar_20260604_patch_v1'
   OR source_version = 'trade_calendar_20260604_patch_v1';
