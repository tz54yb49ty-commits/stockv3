-- V3 20260615 N6 user projection rollback after repaired formal price/amount chain N5 replay.
-- Scope: only the fixed projection run below.
-- Boundary: no N5/N4/N3/N2/N1 mutation, no outbox/inbox/checkpoint mutation,
-- no scheduler/worker, no delivery/push/voice/mobile, no sim/position/PnL,
-- no proposal/order/trade, no real trade, and no old system touch.

BEGIN;

SET LOCAL n6.rollback_user_projection_run_id = 'v3_n6_user_projection_20260615_after_n5_replay_repaired_formal_price_amount_chain_and_n3_coverage_repair_v1';
...
BEGIN
  IF v_run_id IS NULL OR btrim(v_run_id) = '' THEN
    RAISE EXCEPTION 'V3 20260615 N6 repaired formal price amount chain rollback blocked: n6.rollback_user_projection_run_id is not set';
  END IF;

  IF v_run_id <> 'v3_n6_user_projection_20260615_after_n5_replay_repaired_formal_price_amount_chain_and_n3_coverage_repair_v1' THEN
    RAISE EXCEPTION 'V3 20260615 N6 repaired formal price amount chain rollback blocked: unexpected run id %', v_run_id;
  END IF;
  ...
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'V3 20260615 N6 repaired formal price amount chain rollback blocked: expected 1 user_projection_run row for %, found %', v_run_id, v_count;
  END IF;
  ...
  IF v_source_action_run_id <> 'v3_n5_action_replay_20260615_after_n4_repaired_formal_price_amount_chain_and_n3_coverage_repair_v1' THEN
    RAISE EXCEPTION 'V3 20260615 N6 repaired formal price amount chain rollback blocked: unexpected source_action_run_id % for run %', v_source_action_run_id, v_run_id;
  END IF;
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'V3 20260615 N6 repaired formal price amount chain rollback blocked: user_signal_decision has % rows linked to run %', v_count, v_run_id;
  END IF;
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'V3 20260615 N6 repaired formal price amount chain rollback blocked: user_sim_order has % rows linked to run %', v_count, v_run_id;
  END IF;

  SELECT count(*) INTO v_count FROM user_sim_trade WHERE sim_run_id = v_run_id;
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'V3 20260615 N6 repaired formal price amount chain rollback blocked: user_sim_trade has % rows linked to run %', v_count, v_run_id;
  END IF;

  SELECT count(*) INTO v_count FROM user_sim_position WHERE sim_run_id = v_run_id;
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'V3 20260615 N6 repaired formal price amount chain rollback blocked: user_sim_position has % rows linked to run %', v_count, v_run_id;
  END IF;
  ...
  LOOP
    ...
    IF v_count <> 0 THEN
      RAISE EXCEPTION 'V3 20260615 N6 repaired formal price amount chain rollback blocked: linked % refs found in %, rows=%, run=%', v_guard.category, v_guard.table_name, v_count, v_run_id;
    END IF;
  END LOOP;
  ...
  IF v_deleted_runs <> 1 THEN
    RAISE EXCEPTION 'V3 20260615 N6 repaired formal price amount chain rollback blocked: expected to delete 1 projection run, deleted %', v_deleted_runs;
  END IF;

  RAISE NOTICE 'V3 20260615 N6 repaired formal price amount chain rollback completed for %, notification_rows=%, card_rows=%, projection_rows=%, run_rows=%',
