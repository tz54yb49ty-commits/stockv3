-- Restore the exact 046 apply definition and ACL; preserve all business history.
-- REVIEW ONLY: do not execute without a separate rollback gate.

BEGIN;

REVOKE ALL ON FUNCTION public.n6_executor_freeze_next_stop_loss(text) FROM n6_virtual_executor;
REVOKE ALL ON FUNCTION public.n6_executor_evaluate_next_stop_loss(text) FROM n6_virtual_executor;
DROP FUNCTION IF EXISTS public.n6_executor_freeze_next_stop_loss(text);
DROP FUNCTION IF EXISTS public.n6_executor_evaluate_next_stop_loss(text);
