-- N1 official daily 20260602 rollback.
-- Scope: official_daily_ingest_20260602_v1.
-- Safe only before condition source / N2 / N3 / N4 / N5 / N6 downstream refs.
-- This rollback does not touch trade calendar, condition source, outbox/inbox/checkpoint,
-- N2-N6 facts, workers, old system, delivery, notification, sim, position, or real trade.

BEGIN;

DO $$
DECLARE
  v_batch_id text := 'official_daily_ingest_20260602_v1';
  v_stock_source_version text := 'stock_daily_20260602_v1';
  v_index_source_version text := 'index_daily_20260602_v1';
  v_board_source_version text := 'board_daily_20260602_v1';
  v_condition_batch_id text := 'condition_source_activation_20260602_v1';
...
BEGIN
  ...
  WHERE source_action_run_id LIKE '%' || v_condition_run_prefix || '%'
     ...
     OR quality_summary_json::text LIKE '%' || v_stock_source_version || '%'
     OR quality_summary_json::text LIKE '%' || v_index_source_version || '%'
     OR quality_summary_json::text LIKE '%' || v_board_source_version || '%';
  ...
  IF v_outbox_refs <> 0
     ...
     OR v_n6_refs <> 0 THEN
    RAISE EXCEPTION
      'Refusing N1 official daily 20260602 rollback: outbox %, inbox %, checkpoint %, condition_source %, N2 %, N3 %, N4 %, N5 %, N6 %',
      v_outbox_refs,
      v_inbox_refs,
      v_checkpoint_refs,
      v_condition_source_refs,
      v_n2_refs,
      v_n3_refs,
      v_n4_refs,
      v_n5_refs,
...
WHERE scope_key = '20260602'
  AND source_batch_id = 'official_daily_ingest_20260602_v1'
...
WHERE trade_date = '20260602'
  AND source_batch_id = 'official_daily_ingest_20260602_v1'
  AND source_version = 'stock_daily_20260602_v1';

DELETE FROM index_daily_bar_fact
WHERE trade_date = '20260602'
  AND source_batch_id = 'official_daily_ingest_20260602_v1'
  AND source_version = 'index_daily_20260602_v1';

DELETE FROM board_daily_bar_fact
WHERE trade_date = '20260602'
  AND source_batch_id = 'official_daily_ingest_20260602_v1'
  AND source_version = 'board_daily_20260602_v1';

DELETE FROM common_quality_gate_result
WHERE source_batch_id = 'official_daily_ingest_20260602_v1'
...
WHERE batch_id = 'official_daily_ingest_20260602_v1'
   OR source_version IN (
