-- N5 rollback supersession for N4 Y-amount semantic repair rerun.
-- Execute only after explicit approval from:
--   docs/N5_ROLLBACK_SQL_SUPERSESSION_FOR_N4_Y_AMOUNT_SEMANTIC_REPAIR_RERUN_PREFLIGHT.json
-- Supersedes:
--   sql/N5_action_after_repaired_n2_n3_n4_full_scope_pass_rollback.sql
-- Reason:
--   The superseded rollback hard-failed on non-scoped N4 checkpoints using
--   partition_key overlap only. This SQL keeps the hard-fail but scopes true
--   non-scoped checkpoint refs to payload/source/action refs or last_event_id /
--   last_outbox_id pointing at this exact N4 source event set.
-- Scope:
...
--   projection, voice, sim, mobile, position/order/trade, or old-system tables.

BEGIN;

\set action_run_id 'action_consumer_execute_20260617_until_1352_after_repaired_n2_n3_n4_full_scope_pass__trigger_action_confirmation_metric_execute_20260617_until_1352__condition_layer_20260616_source_20260616_for_20260617_semantic_repair_v1'
\set source_trigger_run_id 'trigger_action_confirmation_metric_execute_20260617_until_1352__condition_layer_20260616_source_20260616_for_20260617_semantic_repair_v1'
...
BEGIN
  ...
  WITH scoped_n5_events AS (
    SELECT event_id, outbox_id, partition_key
    FROM common_event_outbox
    WHERE source_layer = 'N5_action'
      AND source_run_id = v_action_run_id
    UNION ALL
    SELECT event_id, first_outbox_id AS outbox_id, partition_key
...
UNION ALL
SELECT 'common_event_checkpoint_n5_consumer_scoped_refs', count(*)
FROM common_event_consumer_checkpoint cp
