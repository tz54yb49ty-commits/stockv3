-- Exact rollback for N6 migration 075.
-- Restores the 066 claim-next function and deletes no history.
...
BEGIN
  ...
  IF function_oid IS NULL THEN
    RAISE EXCEPTION '075_rollback_required_function_missing';
  END IF;
  ...
  IF function_proc.owner_name <> current_user
    ...
    RAISE EXCEPTION '075_rollback_baseline_definition_drift';
  END IF;
END
$preflight$;
...
BEGIN
  ...
  old_text := $claim_quote_ready_075$      )
      AND EXISTS (
        SELECT 1
...
$claim_fifo_066$;
  ...
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '075_rollback_claim_rewrite_mismatch';
  END IF;
...
BEGIN
  ...
  IF function_oid IS NULL
    ...
    RAISE EXCEPTION '075_rollback_postflight_definition_drift';
  END IF;
  ...
  IF expected_execute IS DISTINCT FROM true OR unexpected_execute THEN
    RAISE EXCEPTION '075_rollback_postflight_acl_drift';
  END IF;
END
$postflight$;
