-- Restore the exact pre-087 N6 stop-loss evaluator definition.
...
BEGIN
  IF current_user <> 'ashare_v3_user' THEN
    RAISE EXCEPTION '087_rollback_owner_execution_required';
  END IF;
  IF function_oid IS NULL THEN
    RAISE EXCEPTION '087_rollback_stop_loss_evaluator_missing';
  END IF;
  ...
  IF function_proc.owner_name <> 'ashare_v3_user'
    ...
    RAISE EXCEPTION '087_rollback_stop_loss_evaluator_drift';
  END IF;
  ...
  PERFORM pg_catalog.set_config(
    'n6.migration_087_rollback_business_summary',
...
BEGIN
  ...
  IF pg_catalog.strpos(source_text, old_text) = 0
    ...
    RAISE EXCEPTION '087_rollback_rewrite_scope_drift';
  END IF;
...
DECLARE
  ...
  before_summary jsonb := pg_catalog.current_setting(
    'n6.migration_087_rollback_business_summary', false
  )::jsonb;
...
BEGIN
  ...
  IF function_proc.owner_name <> 'ashare_v3_user'
    ...
    RAISE EXCEPTION '087_rollback_postflight_drift';
  END IF;
  ...
  IF after_summary IS DISTINCT FROM before_summary THEN
    RAISE EXCEPTION '087_rollback_unexpected_business_dml';
  END IF;
END
$postflight$;
