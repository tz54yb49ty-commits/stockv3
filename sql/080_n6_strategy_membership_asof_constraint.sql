-- Allow N6 strategy matches to freeze an authoritative as-of membership date.
...
BEGIN
  IF pg_catalog.current_database() IS DISTINCT FROM 'ashare_v3'
    ...
    RAISE EXCEPTION '080 owner migration identity rejected';
  END IF;

  IF pg_catalog.to_regclass(
       'public.n6_strategy_match_projection'
     ) IS NULL THEN
    RAISE EXCEPTION '080 strategy projection table missing';
  END IF;
  ...
  IF NOT FOUND
     OR target_constraint.relkind <> 'r'
     OR target_constraint.owner_name <> 'ashare_v3_user' THEN
    RAISE EXCEPTION '080 strategy projection table authority rejected';
  END IF;
  ...
  IF NOT FOUND
    ...
    RAISE EXCEPTION '080 membership source date column contract rejected';
  END IF;
  ...
  IF NOT FOUND
    ...
    RAISE EXCEPTION '080 original membership constraint drift';
  END IF;

  IF EXISTS (
    SELECT 1
    FROM public.n6_strategy_match_projection projection
    WHERE projection.membership_source_trade_date IS NULL
       OR projection.membership_source_trade_date > projection.trade_date
  ) THEN
    RAISE EXCEPTION '080 existing membership date data rejected';
  END IF;
END
$preflight$;
...
ALTER TABLE public.n6_strategy_match_projection
  ...
  CHECK (
    membership_source_trade_date IS NOT NULL
    AND membership_source_trade_date <= trade_date
  );
...
BEGIN
  ...
  IF NOT FOUND
    ...
    RAISE EXCEPTION '080 membership as-of constraint postflight failed';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '080 membership date data postflight failed';
  END IF;
END
$postflight$;
