-- N3 lineage refresh for N2 20260615 v2 rollback plan.
-- Scope: only new v2 N3 subscription control rows and new v2 A1 previous-day preload rows.
-- Preserves old v1 lineage rows and all N2/N4/N5/N6 facts.
-- Not executed by contract/preflight/final gate.

\set ON_ERROR_STOP on
\set subscription_run_id 'market_data_subscription_20260616_condition_layer_20260615_source_20260615_for_20260616_v2'
\set preload_run_id 'previous_day_minute_preload_20260615_for_20260616__market_data_subscription_20260616_condition_layer_20260615_source_20260615_for_20260616_v2'
\set source_condition_run_id 'condition_layer_20260615_source_20260615_for_20260616_v2'
...
DO $$
BEGIN
  IF current_setting('ashare_v3.allow_n3_lineage_refresh_20260615_v2_rollback', true) IS DISTINCT FROM 'true' THEN
    RAISE EXCEPTION 'Refusing N3 lineage refresh v2 rollback: set ashare_v3.allow_n3_lineage_refresh_20260615_v2_rollback=true after runtime_control approval';
  END IF;
END $$;

DO $$
DECLARE
  v_subscription_run_id TEXT := 'market_data_subscription_20260616_condition_layer_20260615_source_20260615_for_20260616_v2';
  v_preload_run_id TEXT := 'previous_day_minute_preload_20260615_for_20260616__market_data_subscription_20260616_condition_layer_20260615_source_20260615_for_20260616_v2';
  v_count BIGINT;
  v_table TEXT;
...
BEGIN
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'Refusing N3 lineage refresh v2 rollback: downstream/worker flags set for target runs, count=%', v_count;
  END IF;
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'Refusing N3 lineage refresh v2 rollback: outbox refs exist, count=%', v_count;
  END IF;
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'Refusing N3 lineage refresh v2 rollback: inbox refs exist, count=%', v_count;
  END IF;
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'Refusing N3 lineage refresh v2 rollback: checkpoint refs exist, count=%', v_count;
  END IF;
  ...
  FOREACH v_table IN ARRAY ARRAY[
    ...
    IF to_regclass('public.' || v_table) IS NOT NULL THEN
      ...
      IF v_count <> 0 THEN
        RAISE EXCEPTION 'Refusing N3 lineage refresh v2 rollback: downstream table % references target runs, count=%', v_table, v_count;
      END IF;
    END IF;
  END LOOP;
END $$;
