-- N6 AI Agent v1 execution compatibility (quote, stop, and executor).
...
BEGIN
  IF pg_catalog.to_regprocedure(
    ...
    RAISE EXCEPTION '057 required 055 risk function missing';
  END IF;

  FOREACH function_name IN ARRAY ARRAY[
    'public.n6_quote_writer_scope(timestamp with time zone)',
    'public.n6_quote_writer_save_run(bigint,text,timestamp with time zone,text,integer,integer,integer,timestamp with time zone,timestamp with time zone,jsonb,jsonb)',
  ...
  LOOP
    ...
    IF function_owner <> current_user
      ...
      RAISE EXCEPTION '057 function authority drift: %', function_name;
    END IF;
  END LOOP;
  ...
  WHERE role.rolname = 'n6_ai_agent'
    ...
    AND pg_catalog.has_table_privilege(
          role.oid, c.oid, required.privilege_name
        )
  LIMIT 1;
  IF FOUND THEN
    RAISE EXCEPTION '057 AI role direct relation privilege rejected: %.%',
  ...
  LIMIT 1;
  IF FOUND THEN
    RAISE EXCEPTION '057 AI role direct sequence privilege rejected: %.%',
      leaked_privilege.relname, leaked_privilege.privilege_name;
  END IF;
END
$preflight$;
...
BEGIN
  IF p_principal_id IS NULL
     OR p_principal_id <= 0
     OR p_principal_type NOT IN ('admin', 'human_user', 'ai_user')
...
BEGIN
  ...
  SELECT p.owner_user_id, ai.ai_user_id
    INTO owner_user_id, actor_ai_user_id
  ...
  IF (
       position_row.principal_type IN ('admin', 'human_user')
       AND (owner_user_id IS NULL OR actor_ai_user_id IS NOT NULL)
     )
     OR (
       position_row.principal_type = 'ai_user'
       AND (owner_user_id IS NOT NULL OR actor_ai_user_id IS NULL)
     )
     OR position_row.principal_type NOT IN ('admin', 'human_user', 'ai_user') THEN
    RETURN pg_catalog.jsonb_build_object(
      'ok', true, 'status', 'not_ready', 'reason', 'principal_actor_not_ready'
    );
  END IF;
  ...
  INSERT INTO public.n6_virtual_trade_proposal (
    ...
    CASE
      WHEN position_row.principal_type = 'ai_user'
        THEN 'n6_ai_agent_execution_compat_057_v1'
      ELSE 'n6_virtual_stop_loss_049_v1'
    END,
    CASE
      WHEN position_row.principal_type = 'ai_user'
        THEN 'n6_ai_agent_execution_compat_057_v1'
  ...
  LOOP
    SELECT
      ...
      pg_catalog.has_function_privilege(
        expected_role, function_name, 'EXECUTE'
      ),
      EXISTS (
        SELECT 1
        FROM pg_catalog.pg_roles role
    ...
    IF public_execute
      ...
      RAISE EXCEPTION '057 function ACL mismatch: %', function_name;
    END IF;
  END LOOP;
END
$postcheck$;
