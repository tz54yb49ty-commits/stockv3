-- N6 B-track confirmed manual BUY claim scope fix.
...
BEGIN
  ...
  LOOP
    ...
    IF function_oid IS NULL THEN
      RAISE EXCEPTION '065a_required_function_missing: %',
        expected.signature;
    END IF;
    ...
    IF function_proc.owner_name <> current_user
      ...
      RAISE EXCEPTION '065a_baseline_definition_drift: %',
        expected.signature;
    END IF;
    ...
    IF expected_execute IS DISTINCT FROM true
      ...
      RAISE EXCEPTION '065a_baseline_acl_drift: %',
        expected.signature;
    END IF;
  END LOOP;
  ...
  IF function_proc.owner_name <> current_user
    ...
    RAISE EXCEPTION '065a_scope_helper_definition_drift';
  END IF;
END
$preflight$;
...
BEGIN
  ...
  old_text := $old_next$WHERE p.proposal_status = 'confirmed'
      AND p.expires_at > pg_catalog.now()$old_next$;
  new_text := $new_next$WHERE p.proposal_status = 'confirmed'
      AND (
  ...
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '065a_next_claim_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, new_text);
  EXECUTE pg_catalog.format(
  ...
  LOOP
    ...
    IF function_oid IS NULL
       ...
       OR function_proc.proconfig IS DISTINCT FROM
          ARRAY['search_path=pg_catalog']::text[]
       OR actual_sha <> expected.source_sha THEN
      RAISE EXCEPTION '065a_postflight_definition_drift: %',
        expected.signature;
    END IF;
    ...
    IF expected_execute IS DISTINCT FROM true
       OR unexpected_execute THEN
      RAISE EXCEPTION '065a_postflight_acl_drift: %',
        expected.signature;
    END IF;
  END LOOP;
END
