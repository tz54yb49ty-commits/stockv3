-- N5 full-day trigger-state closed-loop runtime scoped superseding rollback.
--
-- Execute only after an explicitly approved N5 rollback execute gate.
--
-- Scope:
--   action_run_id:
--     action_consumer_dry_run_20260617_full_day_state_closed_loop__trigger_action_confirmation_metric_execute_20260617_full_day_after_n3_full_day_b2_pass__condition_layer_20260616_source_20260616_for_20260617_semantic_repair_v1
...
\set action_run_id 'action_consumer_dry_run_20260617_full_day_state_closed_loop__trigger_action_confirmation_metric_execute_20260617_full_day_after_n3_full_day_b2_pass__condition_layer_20260616_source_20260616_for_20260617_semantic_repair_v1'
\set source_trigger_run_id 'trigger_action_confirmation_metric_execute_20260617_full_day_after_n3_full_day_b2_pass__condition_layer_20260616_source_20260616_for_20260617_semantic_repair_v1'
...
BEGIN
  ...
  WITH scoped_n5_events AS (
    ...
    WHERE source_layer = 'N5_action'
      AND source_run_id = v_action_run_id
    UNION ALL
    SELECT event_id, NULL::bigint AS outbox_id, partition_key
  ...
  WHERE consumer_name = v_consumer_name
    ...
    AND source_run_id = v_source_trigger_run_id
    AND event_type = 'TriggerPendingMarketData';
  IF v_count > 0 THEN
    RAISE EXCEPTION 'N5 rollback blocked: scoped N4 inbox unexpectedly contains TriggerPendingMarketData rows (%)', v_count;
  END IF;
