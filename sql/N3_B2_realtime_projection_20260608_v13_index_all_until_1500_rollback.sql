...
SELECT set_config('app.n3_b2_projection_run_id', 'realtime_projection_metric_20260608_until_1500__realtime_daily_snapshot_20260608__market_data_subscription_20260608_condition_layer_20260605_to_20260608_v13_index_all_execute', false);
...
BEGIN
  ...
  IF outbox_refs <> 0
    ...
    RAISE EXCEPTION 'N3-B2 rollback blocked for %, outbox=%, delivered_or_delivering=%, inbox=%, checkpoint=%, trigger_state=%, trigger_match=%, action=%, n6=%, downstream_or_worker=%',
      target_run_id, outbox_refs, delivered_or_delivering_outbox_refs, inbox_refs, checkpoint_refs, trigger_state_refs, trigger_match_refs, action_refs, n6_refs, downstream_flags;
  END IF;
END $$;
