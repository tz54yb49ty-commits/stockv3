-- V3 20260615 N5 replay after N3 action-confirmation metric rollback.
-- Execute only after an explicitly approved N5 replay execute needs rollback.
-- Scope:
...
-- Boundary:
--   Deletes only scoped N5 replay rows and this N5 consumer's inbox/checkpoint
--   rows for the scoped N4 source run. It preserves N4 trigger facts, N4
--   outbox status, N3 projection/metric facts, N6/user rows, scheduler state,
--   voice, mobile, sim, position, and true-trade tables.

BEGIN;

\set action_run_id 'n5_action_bounded_20260615_after_n3_action_confirmation_metric_until_1000_v1'
\set source_trigger_run_id 'n4_production_semantic_replay_20260615_market_snapshot_updated_until_1000'
\set consumer_name 'n5_action_bounded_consumer_20260615_after_n3_metric_until_1000_v1'
...
BEGIN
  ...
  -- Other consumers' checkpoints on the preserved N4 source run are not
  -- downstream refs to this N5 action run. They must remain untouched, but they
  -- must not block rollback of this scoped N5 consumer.
