-- Harden the two published 042 scope-write functions with database-side
-- current approved-source validation. Existing signatures, owners and ACLs
-- are preserved by CREATE OR REPLACE FUNCTION.
...
BEGIN
  ...
  IF p_asset_kind = 'stock' THEN
    ...
    SELECT (authority->>'principal_id')::bigint,authority->>'principal_type',(authority->>'user_id')::bigint,
           'stock',p_identity_key,'single_row',
           pg_catalog.jsonb_build_object('identity_key',p_identity_key,'for_trade_date',p_for_trade_date),
  ...
  ELSIF p_asset_kind = 'index' THEN
    ...
    SELECT (authority->>'principal_id')::bigint,authority->>'principal_type',(authority->>'user_id')::bigint,
           'index',p_identity_key,'single_row',
           pg_catalog.jsonb_build_object('identity_key',p_identity_key,'for_trade_date',p_for_trade_date),
           false,'active',NULL,pg_catalog.now(),pg_catalog.now()
    FROM approved_source
    ON CONFLICT (principal_id,principal_type,user_id,asset_kind,identity_key)
  ...
  ELSE
    ...
    SELECT (authority->>'principal_id')::bigint,authority->>'principal_type',(authority->>'user_id')::bigint,
           'board',p_identity_key,'single_row',
           pg_catalog.jsonb_build_object('identity_key',p_identity_key,'for_trade_date',p_for_trade_date),
           false,'active',NULL,pg_catalog.now(),pg_catalog.now()
    FROM approved_source
    ON CONFLICT (principal_id,principal_type,user_id,asset_kind,identity_key)
