-- N4 projection matcher 20260608 until 15:00 v4 repair retry scoped rollback.
-- Scope: target N4 retry run only.
-- Preserves N3 snapshot/projection facts, N3 market snapshot event outbox, and N1/N2 facts.

BEGIN;

\set target_run_id 'trigger_projection_matcher_execute_20260608_v13_index_all_until_1500_v4_repair_retry'
...
BEGIN
  ...
  IF v_run_id <> 'trigger_projection_matcher_execute_20260608_v13_index_all_until_1500_v4_repair_retry' THEN
  ...
  SELECT count(*) INTO v_total_count
   ...
   WHERE source_layer = 'N4_trigger'
     AND source_run_id = v_run_id
     AND consumer_name <> 'n4_projection_matcher_consumer_v1_until_1500_reprocess';
  IF v_total_count <> 0 THEN
  ...
  LOOP
    ...
    ELSIF v_table_name = 'common_event_inbox' THEN
      SELECT count(*) INTO v_scoped_count
        FROM common_event_inbox
       WHERE consumer_name = 'n4_projection_matcher_consumer_v1_until_1500_reprocess'
