-- N1 trade calendar 20260529 patch rollback draft.
-- Scope: trade_calendar_20260529_patch_v1.
-- This rollback does not touch 20260528 calendar, daily fact, condition source,
...
BEGIN
  ...
  WHERE a.data_domain = 'common'
    ...
    AND a.scope_key = 'SSE:20260529'
    AND a.source_batch_id = 'trade_calendar_20260529_patch_v1'
    AND a.source_version = 'trade_calendar_20260529_patch_v1'
  ...
  IF missing_previous_batch_count > 0 THEN
    RAISE EXCEPTION 'Refusing 20260529 calendar patch rollback: previous_source_version cannot be resolved';
  END IF;
END $$;
...
SET source_version = a.previous_source_version,
    ...
    activated_at = now(),
    activated_by = 'rollback:n1_trade_calendar_20260529_patch'
WHERE a.data_domain = 'common'
  AND a.data_type = 'trade_calendar'
  AND a.scope_key = 'SSE:20260529'
  AND a.source_batch_id = 'trade_calendar_20260529_patch_v1'
...
WHERE data_domain = 'common'
  ...
  AND source_batch_id = 'trade_calendar_20260529_patch_v1'
  AND source_version = 'trade_calendar_20260529_patch_v1'
...
WHERE source_batch_id = 'trade_calendar_20260529_patch_v1'
   OR source_version = 'trade_calendar_20260529_patch_v1';

DELETE FROM common_ingest_batch
WHERE batch_id = 'trade_calendar_20260529_patch_v1'
   OR source_version = 'trade_calendar_20260529_patch_v1';
