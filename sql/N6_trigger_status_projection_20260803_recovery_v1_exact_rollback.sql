-- Exact rollback for the frozen empty-baseline 20260803 trigger-status recovery.
...
SELECT pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'n6_trigger_status_projection_v1:trigger-status:20260803', 0
  )
);
...
BEGIN
  IF pg_catalog.current_database() IS DISTINCT FROM 'ashare_v3'
    ...
    RAISE EXCEPTION '20260803 exact rollback database or owner identity rejected';
  END IF;
  ...
  IF target_oid IS NULL
    ...
    RAISE EXCEPTION '20260803 exact rollback migration 089 object identity missing';
  END IF;
  ...
  IF table_owner IS DISTINCT FROM 'ashare_v3_user'
    ...
    RAISE EXCEPTION '20260803 exact rollback migration 089 schema identity drift';
  END IF;
  ...
  WITH target_scope AS (
    ...
    WHERE outbox.outbox_id BETWEEN 4107628 AND 4110567
      AND outbox.trade_date = '20260803'
  ...
  IF target_inbox_count <> 1769
     OR target_date_inbox_count <> 1769
     OR eligible_count <> 863
     OR executed_count <> 587
     OR updated_count <> 8
     OR invalidated_count <> 311
     OR minimum_outbox_id IS DISTINCT FROM 4107628
     OR maximum_outbox_id IS DISTINCT FROM 4110567
     OR malformed_inbox_count <> 0 THEN
    RAISE EXCEPTION
      '20260803 exact rollback input/inbox scope drift: target=%, date=%, eligible=%, executed=%, updated=%, invalidated=%, min=%, max=%, malformed=%',
  ...
  IF unmatched_updated_count <> 0
     ...
     OR distinct_invalidated_entry_count <> 311
     OR expected_active_count <> 552 THEN
    RAISE EXCEPTION
      '20260803 exact rollback event lineage drift: unmatched_updated=%, unmatched_invalidated=%, distinct_invalidated=%, expected_active=%',
  ...
  WHERE current_row.consumer_name = 'n6_trigger_status_projection_v1'
    AND current_row.projection_run_id =
        'n6_trigger_status_projection_20260803_recovery_v1'
    AND current_row.trade_date = '20260803';
  ...
  IF target_current_count <> 552 OR current_mismatch_count <> 0 THEN
    RAISE EXCEPTION
      '20260803 exact rollback projection success state drift: current=%, mismatches=%',
      target_current_count, current_mismatch_count;
  END IF;

  SELECT count(*),
         count(*) FILTER (
           WHERE checkpoint.last_outbox_id IS DISTINCT FROM 4110567
  ...
  WHERE checkpoint.consumer_name = 'n6_trigger_status_projection_v1'
    AND checkpoint.partition_key = 'trigger-status:20260803'
    AND checkpoint.source_layer = 'N5_action';

  IF exact_checkpoint_count <> 1 OR checkpoint_mismatch_count <> 0 THEN
    RAISE EXCEPTION
      '20260803 exact rollback checkpoint drift: count=%, mismatches=%',
  ...
  WHERE inbox.consumer_name = 'n6_trigger_status_projection_v1'
    AND inbox.source_layer = 'N5_action'
    AND inbox.event_id = outbox.event_id
    AND outbox.outbox_id BETWEEN 4107628 AND 4110567
    AND outbox.trade_date = '20260803'
    AND outbox.source_layer = 'N5_action'
    AND outbox.event_type IN (
      'ActionEligible', 'ActionExecuted',
  ...
  IF deleted_current_count <> 552
     OR deleted_inbox_count <> 1769
     OR deleted_checkpoint_count <> 1 THEN
    RAISE EXCEPTION
      '20260803 exact rollback effect count drift: current=%, inbox=%, checkpoint=%',
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '20260803 exact rollback postflight target residue';
  END IF;
  ...
  IF after_outbox_range_count IS DISTINCT FROM before_outbox_range_count
    ...
    RAISE EXCEPTION '20260803 exact rollback outbox or migration 089 postflight drift';
  END IF;

  RAISE NOTICE
    '20260803 exact rollback PASS: current_deleted=%, inbox_deleted=%, checkpoint_deleted=%, outbox_updates=0',
    deleted_current_count, deleted_inbox_count, deleted_checkpoint_count;
END
$exact_rollback$;
