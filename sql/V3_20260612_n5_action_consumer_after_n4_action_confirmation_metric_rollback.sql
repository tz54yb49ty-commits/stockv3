...
-- Scope:
--   action_run_id: v3_n5_action_consumer_20260612_from_n4_action_confirmation_metric_after_n3_writer_v1
...
-- Boundary:
--   Deletes only N5 action-layer rows and reviewed stale N5 consumers'
--   inbox/checkpoint rows for the scoped N4 source run. It does not mutate N4
--   trigger facts, N4 outbox status, N3 facts, user projection, voice, sim,
--   mobile, or true-trade tables.

BEGIN;

\set action_run_id 'v3_n5_action_consumer_20260612_from_n4_action_confirmation_metric_after_n3_writer_v1'
\set source_trigger_run_id 'v3_n4_action_confirmation_metric_20260612_after_realtime_virtual_metric_writer_v1'
\set consumer_name 'n5_action_consumer_v1'
\set wrapper_consumer_name 'v3_realtime_engine_n5_consumer_20260612'
...
SET LOCAL n5.rollback_wrapper_consumer_name = :'wrapper_consumer_name';
...
DECLARE
  ...
  v_wrapper_consumer_name text := current_setting('n5.rollback_wrapper_consumer_name');
  v_stale_consumer_names text[] := ARRAY[
    current_setting('n5.rollback_consumer_name'),
    current_setting('n5.rollback_wrapper_consumer_name')
  ];
  v_count bigint := 0;
  v_table_name text;
...
BEGIN
  ...
  WHERE source_layer = 'N4_trigger'
    ...
    AND NOT (consumer_name = ANY(v_stale_consumer_names));
  IF v_count > 0 THEN
    RAISE EXCEPTION 'N5 rollback blocked: non-stale consumer inbox refs exist for source_trigger_run_id (%)', v_count;
  END IF;

  -- Non-scoped consumer checkpoints on the preserved N4 source run are not
  -- downstream refs to this N5 action run. They must remain untouched, but they
  -- must not block rollback of reviewed stale N5 consumers' inbox/checkpoint rows.
...
FROM common_event_inbox
WHERE consumer_name = ANY(ARRAY[:'consumer_name', :'wrapper_consumer_name'])
