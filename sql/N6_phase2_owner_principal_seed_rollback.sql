-- N6 Phase 2 owner/principal seed rollback.
-- Do not execute without explicit user confirmation.
-- Scope: delete only n6_principal rows created by the configured seed_run_id.
...
DECLARE
  ...
  v_optional_checks TEXT[][] := ARRAY[
    ...
    ARRAY['n6_ai_evaluation', 'principal_id'],
...
BEGIN
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'rollback blocked: linked principal account refs exist: %', v_count;
  END IF;
  ...
  WHERE principal_id = ANY(v_seed_principal_ids);
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'rollback blocked: linked strategy refs exist: %', v_count;
  END IF;
  ...
  WHERE principal_id = ANY(v_seed_principal_ids);
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'rollback blocked: linked ai user refs exist: %', v_count;
  END IF;
