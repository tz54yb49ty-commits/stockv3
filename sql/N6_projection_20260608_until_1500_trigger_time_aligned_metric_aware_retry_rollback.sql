-- N6 projection shadow execute business rollback for 20260608 until 15:00 trigger-time-aligned metric-aware retry.
...
SET LOCAL n6.rollback_user_projection_run_id = 'user_projection_shadow_20260608_until_1500_trigger_time_aligned_metric_aware_retry__action_consumer_execute_20260608_until_1500_trigger_time_aligned_metric_aware_retry';
...
DECLARE
  ...
  v_count BIGINT;
  v_delivered_or_delivering_outbox_refs BIGINT;
  v_guard RECORD;
...
BEGIN
  ...
  SELECT count(*) INTO v_delivered_or_delivering_outbox_refs
    FROM common_event_outbox
   WHERE source_layer = 'N5_action'
     AND source_run_id = v_source_action_run_id
     AND status IN ('delivering', 'delivered');
  IF v_delivered_or_delivering_outbox_refs <> 0 THEN
    RAISE EXCEPTION 'N6 projection rollback blocked: source N5 outbox has delivered/delivering rows %, source_action_run_id=%',
      v_delivered_or_delivering_outbox_refs, v_source_action_run_id;
  END IF;
