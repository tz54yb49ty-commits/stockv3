-- Scoped rollback for the superseded 20260615 N5 action replay until_1342 v1.
-- This rollback deletes only the scoped N5 action run and the scoped N5
-- consumer's inbox/checkpoint rows for the preserved N4 source run.
-- It intentionally preserves N3 metric facts, N4 trigger facts/outbox status,
-- other consumers' inbox/checkpoint rows, N6/user rows, voice/mobile/sim,
-- position/order/trade rows, and scheduler state.

BEGIN;

\set action_run_id 'n5_action_bounded_20260615_after_n3_action_confirmation_metric_until_1342_v1'
\set source_trigger_run_id 'n4_production_semantic_replay_20260615_market_snapshot_updated_until_1342'
\set consumer_name 'n5_action_bounded_consumer_20260615_after_n3_metric_until_1342_v1'
...
-- Hard-fail guards: all checks run before the first DELETE.
DO $$
DECLARE
...
BEGIN
  ...
  -- Other consumers on the preserved N4 source are reviewed as unrelated
  -- runtime history for this rollback. They are neither deleted nor blocking.
  ...
  LOOP
    ...
    IF v_table_regclass IS NOT NULL THEN
      EXECUTE format(
        'SELECT count(*) FROM %s t WHERE to_jsonb(t)::text LIKE $1',
        v_table_regclass
      )
      INTO v_count
      USING '%' || v_action_run_id || '%';
      IF v_count > 0 THEN
        RAISE EXCEPTION 'N5 rollback blocked: downstream table % has scoped N5 refs (%)', v_table_name, v_count;
      END IF;
    END IF;
  END LOOP;

  PERFORM v_source_trigger_run_id;
END $$;
