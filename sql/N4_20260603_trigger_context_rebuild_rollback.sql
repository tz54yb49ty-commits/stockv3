...
DO $$
DECLARE
  v_run_id TEXT := 'trigger_context_snapshot_20260603_condition_layer_20260602_source_20260602_v1';
  v_count BIGINT;
BEGIN
  ...
  IF to_regclass('public.user_projection_run') IS NOT NULL THEN
    ...
    USING v_run_id;
    IF v_count <> 0 THEN
      RAISE EXCEPTION 'Refusing N4 context rollback: N6 user_projection_run has % rows for %', v_count, v_run_id;
    END IF;
  END IF;
  ...
  IF to_regclass('public.user_signal_projection') IS NOT NULL THEN
    ...
    USING v_run_id;
    IF v_count <> 0 THEN
      RAISE EXCEPTION 'Refusing N4 context rollback: N6 user_signal_projection has % rows for %', v_count, v_run_id;
    END IF;
  END IF;
  ...
  IF to_regclass('public.user_signal_card') IS NOT NULL THEN
    ...
    USING v_run_id;
    IF v_count <> 0 THEN
      RAISE EXCEPTION 'Refusing N4 context rollback: N6 user_signal_card has % rows for %', v_count, v_run_id;
    END IF;
  END IF;
  ...
  IF to_regclass('public.user_notification_queue') IS NOT NULL THEN
    ...
    USING v_run_id;
    IF v_count <> 0 THEN
      RAISE EXCEPTION 'Refusing N4 context rollback: N6 user_notification_queue has % rows for %', v_count, v_run_id;
    END IF;
  END IF;
END $$;

DELETE FROM common_trigger_quality_item WHERE run_id = 'trigger_context_snapshot_20260603_condition_layer_20260602_source_20260602_v1';
DELETE FROM stock_trigger_context_snapshot WHERE run_id = 'trigger_context_snapshot_20260603_condition_layer_20260602_source_20260602_v1';
DELETE FROM index_trigger_context_snapshot WHERE run_id = 'trigger_context_snapshot_20260603_condition_layer_20260602_source_20260602_v1';
DELETE FROM board_trigger_context_snapshot WHERE run_id = 'trigger_context_snapshot_20260603_condition_layer_20260602_source_20260602_v1';
DELETE FROM common_trigger_run WHERE run_id = 'trigger_context_snapshot_20260603_condition_layer_20260602_source_20260602_v1';
