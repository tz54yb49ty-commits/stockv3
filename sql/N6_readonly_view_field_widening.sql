-- N6 readonly view field widening migration draft.
-- Layer: runtime_control draft for N6_user readonly source boundary.
-- Scope: widen N6 readonly views only; no base-table grants, no data writes.
-- Boundary: no N3/N4/N5/N6 action flow, no outbox/inbox/checkpoint, no worker.
-- Do not execute without explicit final gate approval and user confirmation.
...
CREATE OR REPLACE VIEW v_n6_index_membership_fact AS
SELECT
  trade_date,
  index_identity_key,
  stock_identity_key,
...
CREATE OR REPLACE VIEW v_n6_board_membership_fact AS
SELECT
  trade_date,
  board_identity_key,
  stock_identity_key,
