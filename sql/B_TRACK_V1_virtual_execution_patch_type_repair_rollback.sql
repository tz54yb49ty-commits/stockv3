-- B_TRACK_V1 virtual execution patch type repair rollback draft.
-- Scope: restore three zero-value legacy V2 column types before V1 patch.
...
BEGIN
  ...
  IF FOUND THEN
    RAISE EXCEPTION
      'B_TRACK_V1 type repair rollback date type preflight failed: %.% expected %, found %',
...
BEGIN
  ...
  IF v_count > 0 THEN
    RAISE EXCEPTION
      'B_TRACK_V1 type repair rollback blocked: n6_virtual_order.source_for_trade_date has % business value rows',
      v_count;
  END IF;
  ...
  IF v_count > 0 THEN
    RAISE EXCEPTION
      'B_TRACK_V1 type repair rollback blocked: n6_virtual_position_event.trade_date has % business value rows',
      v_count;
  END IF;
  ...
  IF v_count > 0 THEN
    RAISE EXCEPTION
      'B_TRACK_V1 type repair rollback blocked: n6_virtual_position_event.available_date has % business value rows',
      v_count;
  END IF;
END
$$;
...
BEGIN
  ...
  IF FOUND THEN
    RAISE EXCEPTION
      'B_TRACK_V1 type repair rollback dependency guard failed: % %.% used by %',
  ...
  IF FOUND THEN
    RAISE EXCEPTION
      'B_TRACK_V1 type repair rollback dependency guard failed: % reference exists in %',
      v_dependency.dependency_kind,
      v_dependency.object_name;
  END IF;
END
