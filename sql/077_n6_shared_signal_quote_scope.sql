-- N6 shared-signal quote scope compatibility for human/admin proposals.
-- REVIEWED MIGRATION: execute only through a separately authorized N6 gate.
-- Function-only migration; it performs no proposal, order, trade, cash,
-- position, lot or quote DML.
...
BEGIN
  ...
  LOOP
    ...
    IF function_oid IS NULL THEN
      RAISE EXCEPTION '077_required_function_missing: %', expected.signature;
    END IF;

    SELECT function_row.prosrc, function_row.prosecdef,
           function_row.provolatile, function_row.proparallel,
           function_row.proconfig, function_owner.rolname AS owner_name
      INTO function_proc
    ...
    IF function_proc.owner_name <> current_user
      ...
      RAISE EXCEPTION '077_baseline_definition_drift: %', expected.signature;
    END IF;
    ...
    IF expected_execute IS DISTINCT FROM true OR unexpected_execute THEN
      RAISE EXCEPTION '077_baseline_acl_drift: %', expected.signature;
    END IF;
  END LOOP;
  ...
  PERFORM pg_catalog.set_config(
    'n6.migration_077_business_summary', business_summary::text, true
  );
END
$preflight$;
...
BEGIN
  ...
  WHERE function_row.oid =
        'public.n6_quote_writer_scope(timestamptz)'::regprocedure;

  old_text := $human_owned_source_068$    LEFT JOIN public.user_signal_projection source
...
$human_owned_source_068$;
  new_text := $human_shared_source_077$    LEFT JOIN public.n6_ai_shared_signal_projection shared_source
...
$human_shared_gate_077$;
  ...
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '077_human_gate_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, new_text);
...
BEGIN
  ...
  LOOP
    ...
    IF function_oid IS NULL
       ...
       OR function_proc.proconfig IS DISTINCT FROM
          ARRAY['search_path=pg_catalog']::text[]
       OR actual_sha <> expected.source_sha THEN
      RAISE EXCEPTION '077_postflight_definition_drift: %', expected.signature;
    END IF;
    SELECT
    ...
    IF expected_execute IS DISTINCT FROM true OR unexpected_execute THEN
      RAISE EXCEPTION '077_postflight_acl_drift: %', expected.signature;
    END IF;
  END LOOP;

  before_summary := pg_catalog.current_setting(
    'n6.migration_077_business_summary', false
  )::jsonb;
  ...
  IF after_summary IS DISTINCT FROM before_summary THEN
    RAISE EXCEPTION '077_business_summary_drift';
  END IF;
END
$postflight$;
