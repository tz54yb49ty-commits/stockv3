-- N6 B-track regular-session manual virtual BUY restoration.
-- REVIEWED MIGRATION: execute only through a separately authorized N6 gate.
-- This migration changes no proposal, order, trade, cash, position or lot rows.
...
BEGIN
  IF pg_catalog.to_regprocedure(
    ...
    RAISE EXCEPTION '066_session_helper_already_exists';
  END IF;
  ...
  LOOP
    ...
    IF function_oid IS NULL THEN
      RAISE EXCEPTION '066_required_function_missing: %',
        expected.signature;
    END IF;
    ...
    IF function_proc.owner_name <> current_user
      ...
      RAISE EXCEPTION '066_baseline_definition_drift: %',
        expected.signature;
    END IF;
  END LOOP;
END
...
AS $function$
  WITH current_clock AS (
    SELECT pg_catalog.clock_timestamp()
             AT TIME ZONE 'Asia/Shanghai' AS shanghai_now
  )
  SELECT
    (
      SELECT count(*)
      ...
      WHERE calendar.trade_date = pg_catalog.to_char(
              current_clock.shanghai_now,
              'YYYYMMDD'
            )
        AND calendar.is_open = true
    ) = 1
    AND (
      current_clock.shanghai_now::time
        BETWEEN time '09:30:00' AND time '11:30:00'
      OR current_clock.shanghai_now::time
        BETWEEN time '13:00:00' AND time '15:00:00'
    )
  FROM current_clock
$function$;
...
BEGIN
  ...
  old_text := $create_session_065b$  IF NOT (
  ...
  END IF;
$create_session_065b$;
  new_text := $create_session_066$  IF NOT public.n6_btrack_regular_trade_session_open() THEN
    RETURN pg_catalog.jsonb_build_object(
      'ok', false, 'status', 'not_ready',
      'error', 'outside_trading_session'
    );
...
$create_session_066$;
  ...
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '066_create_session_rewrite_mismatch';
  END IF;
  ...
  old_text := $confirm_session_065b$  IF NOT FOUND THEN RETURN pg_catalog.jsonb_build_object('ok',false,'status','not_found','error','proposal_not_found'); END IF;
  IF row_value.proposal_status IN ('pending', 'confirmed')
$confirm_session_065b$;
  new_text := $confirm_session_066$  IF NOT FOUND THEN RETURN pg_catalog.jsonb_build_object('ok',false,'status','not_found','error','proposal_not_found'); END IF;
...
$confirm_session_066$;
  ...
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '066_confirm_session_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, new_text);
  EXECUTE pg_catalog.format(
  ...
  old_text := $claim_explicit_065b$WHERE proposal_id=p_proposal_id AND proposal_status='confirmed' AND (expires_at>pg_catalog.now()$claim_explicit_065b$;
  new_text := $claim_explicit_066$WHERE proposal_id=p_proposal_id AND proposal_status='confirmed' AND public.n6_btrack_regular_trade_session_open() AND (expires_at>pg_catalog.now()$claim_explicit_066$;
  occurrence_count := (
    pg_catalog.length(source_text)
    - pg_catalog.length(pg_catalog.replace(source_text, old_text, ''))
  ) / pg_catalog.length(old_text);
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '066_explicit_claim_session_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, new_text);
  EXECUTE pg_catalog.format(
  ...
  old_text := $claim_next_065b$    WHERE p.proposal_status = 'confirmed'
      AND (
$claim_next_065b$;
  new_text := $claim_next_066$    WHERE p.proposal_status = 'confirmed'
...
$claim_next_066$;
  ...
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '066_next_claim_session_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, new_text);
  EXECUTE pg_catalog.format(
  ...
  old_text := $apply_session_065b$  IF proposal.proposal_status <> 'processing'
...
$apply_session_065b$;
  new_text := $apply_session_066$  IF proposal.proposal_status <> 'processing'
     OR proposal.executor_run_id IS DISTINCT FROM p_executor_run_id THEN
    RETURN pg_catalog.jsonb_build_object('ok', false, 'status', 'not_claimed');
  END IF;
  IF NOT public.n6_btrack_regular_trade_session_open() THEN
...
$apply_session_066$;
  ...
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '066_apply_session_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, new_text);

  old_text := $manual_fill_065b$    IF FOUND THEN
    ...
    END IF;
$manual_fill_065b$;
  new_text := $manual_fill_066$    IF NOT FOUND THEN
      RETURN pg_catalog.jsonb_build_object(
...
$manual_fill_066$;
  ...
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '066_manual_fill_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, new_text);
  ...
  IF occurrence_count <> 15 THEN
    RAISE EXCEPTION '066_apply_policy_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, new_text);
  ...
  LOOP
    ...
    IF function_oid IS NULL
       ...
       OR function_proc.proconfig IS DISTINCT FROM
          ARRAY['search_path=pg_catalog']::text[]
       OR actual_sha <> expected.source_sha THEN
      RAISE EXCEPTION '066_postflight_definition_drift: %',
        expected.signature;
    END IF;
    ...
    IF expected_execute IS DISTINCT FROM true
      ...
      RAISE EXCEPTION '066_postflight_acl_drift: %',
        expected.signature;
    END IF;
  END LOOP;
END
