-- N6 projection shadow execute business rollback for 20260608 until 09:52 metric-aware retry.
-- Generated from sql/N6_projection_20260608_until_1500_metric_aware_retry_rollback.sql with a reviewed scoped run id.
-- Do not execute without explicit user confirmation and a reviewed user_projection_run_id.
--
-- Scope: delete only N6 projection rows created by one future shadow projection execute run.
-- Boundary: no N1-N5 mutation, no N5 outbox status update, no N5 inbox/checkpoint update,
-- no admin/profile/session/watchlist rollback, no sim rollback, no push/voice/mobile rollback,
-- and no real trade rollback.

BEGIN;

SET LOCAL n6.rollback_user_projection_run_id = 'user_projection_shadow_20260608_until_0952_metric_aware_retry__action_consumer_execute_20260608_until_0952_metric_aware_retry';
