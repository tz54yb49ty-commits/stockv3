-- N6 B-track T+1 natural-day unlock migration.
-- REVIEWED MIGRATION: execute only through a separately authorized N6 gate.
-- Rewrites one executor function and changes no proposal, order, trade, cash,
-- position, lot, quote or signal rows.
...
BEGIN
  ...
  LOOP
    ...
    IF function_oid IS NULL THEN
      RAISE EXCEPTION '069_required_function_missing: %',
        expected.signature;
    END IF;
    ...
    IF function_proc.owner_name <> current_user
      ...
      RAISE EXCEPTION '069_baseline_definition_drift: %',
        expected.signature;
    END IF;
    ...
    IF expected_execute IS DISTINCT FROM true
      ...
      RAISE EXCEPTION '069_baseline_acl_drift: %',
        expected.signature;
    END IF;
  END LOOP;
END
...
BEGIN
  ...
  old_text := $next_trade_date_declaration$  next_trade_date date;
$next_trade_date_declaration$;
  ...
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '069_next_trade_date_declaration_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, '');

  old_text := $future_calendar_dependency$    SELECT pg_catalog.to_date(min(trade_date)::text, 'YYYYMMDD') INTO next_trade_date
...
$future_calendar_dependency$;
  ...
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '069_future_calendar_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, '');

  old_text := $future_open_day_lot$      trade_date_date, next_trade_date, fill_quantity, fill_quantity, fill_price,
$future_open_day_lot$;
  new_text := $natural_day_lot$      trade_date_date, trade_date_date + 1, fill_quantity, fill_quantity,
      fill_price,
$natural_day_lot$;
  occurrence_count := (
    pg_catalog.length(source_text)
  ...
  IF occurrence_count <> 1 THEN
    RAISE EXCEPTION '069_lot_available_date_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, new_text);

  old_text := 'n6_btrack_regular_session_manual_buy_066_v1';
  new_text := 'n6_btrack_t1_natural_day_unlock_069_v1';
  occurrence_count := (
    pg_catalog.length(source_text)
    - pg_catalog.length(pg_catalog.replace(source_text, old_text, ''))
  ) / pg_catalog.length(old_text);
  IF occurrence_count <> 15 THEN
    RAISE EXCEPTION '069_apply_policy_rewrite_mismatch';
  END IF;
  source_text := pg_catalog.replace(source_text, old_text, new_text);
...
BEGIN
  ...
  LOOP
    function_oid := pg_catalog.to_regprocedure(expected.signature);
    IF function_oid IS NULL THEN
      RAISE EXCEPTION '069_postflight_function_missing: %',
        expected.signature;
    END IF;
    ...
    IF function_proc.owner_name <> current_user
       ...
       OR function_proc.proconfig IS DISTINCT FROM
          ARRAY['search_path=pg_catalog']::text[]
       OR actual_sha <> expected.source_sha THEN
      RAISE EXCEPTION '069_postflight_definition_drift: %',
        expected.signature;
    END IF;
    ...
    IF expected_execute IS DISTINCT FROM true
       OR unexpected_execute THEN
      RAISE EXCEPTION '069_postflight_acl_drift: %',
        expected.signature;
    END IF;
  END LOOP;
END
