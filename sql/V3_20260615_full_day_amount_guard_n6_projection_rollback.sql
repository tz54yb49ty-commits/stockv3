-- V3 20260615 full-day amount-guard N6 projection scoped rollback.
-- Scope: only rows created by
--   v3_n6_user_projection_20260615_after_n5_amount_guard_fix_until_1500_v1
-- Boundary: no N1-N5 mutation, no N5 outbox status update, no N5 inbox/checkpoint
-- update, no voice/mobile/sim/position/order/real-trade rollback.
...
BEGIN
  IF current_setting('ashare_v3.allow_v3_20260615_full_day_amount_guard_n6_projection_rollback', true) <> 'true' THEN
    RAISE EXCEPTION
      'blocked: set ashare_v3.allow_v3_20260615_full_day_amount_guard_n6_projection_rollback=true before rollback';
  END IF;
  ...
  IF v_count <> 1 THEN
    RAISE EXCEPTION
      'N6 projection rollback blocked: expected 1 user_projection_run row for %, found %',
      v_run_id, v_count;
  END IF;
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION
      'N6 projection rollback blocked: user_signal_decision has % rows for run %',
      v_count, v_run_id;
  END IF;
  ...
  LOOP
    ...
    END IF;
    IF v_source_action_run_id IS NOT NULL
       AND EXISTS (
         SELECT 1 FROM information_schema.columns
          WHERE table_schema = 'public'
            AND table_name = v_guard.table_name
    ...
    IF v_count <> 0 THEN
      RAISE EXCEPTION
        'N6 projection rollback blocked: linked % refs found in %, rows=%, run=%',
  ...
  IF v_deleted_runs <> 1 THEN
    RAISE EXCEPTION
      'N6 projection rollback blocked: expected to delete 1 projection run, deleted %',
      v_deleted_runs;
  END IF;

  RAISE NOTICE
    'N6 projection rollback completed for %, notification_rows=%, card_rows=%, projection_rows=%, run_rows=%',
