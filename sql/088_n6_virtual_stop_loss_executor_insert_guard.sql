-- N6 virtual stop-loss executor proposal-insert guard compatibility repair.
...
DECLARE
  ...
  executor_evaluator_execute boolean;
  unexpected_evaluator_execute boolean;
  business_summary jsonb := '{}'::jsonb;
  relation_name text;
  row_count bigint;
BEGIN
  IF current_user <> 'ashare_v3_user' THEN
    RAISE EXCEPTION '088_owner_execution_required';
  END IF;
  IF guard_oid IS NULL OR evaluator_oid IS NULL THEN
    RAISE EXCEPTION '088_required_function_missing';
  END IF;
  ...
  SELECT EXISTS (
    ...
    WHERE target.oid = evaluator_oid
  ...
  ) INTO executor_evaluator_execute, unexpected_evaluator_execute;
  ...
  IF guard_proc.owner_name <> 'ashare_v3_user'
     ...
     OR evaluator_proc.owner_name <> 'ashare_v3_user'
     OR evaluator_proc.prosecdef IS DISTINCT FROM true
     OR evaluator_proc.provolatile <> 'v'
     OR evaluator_proc.proparallel <> 'u'
     OR evaluator_proc.proconfig IS DISTINCT FROM
        ARRAY['search_path=pg_catalog']::text[]
     OR evaluator_sha <>
        'fe3b0ac7297f24fc0a5925d178ccb1f26e575716baea48dce40ef6b2af0a1443'
     OR executor_evaluator_execute IS DISTINCT FROM true
     OR unexpected_evaluator_execute THEN
    RAISE EXCEPTION '088_function_baseline_drift';
  END IF;
  IF pg_catalog.has_table_privilege(
       'n6_virtual_executor', 'public.n6_virtual_trade_proposal',
       'INSERT,UPDATE,DELETE'
     ) THEN
    RAISE EXCEPTION '088_executor_direct_proposal_dml_detected';
  END IF;
  ...
  PERFORM pg_catalog.set_config(
    'n6.migration_088_business_summary', business_summary::text, true
  );
END
$preflight$;
...
BEGIN
  ...
  old_text := $guard_executor_insert_078$  ELSIF TG_OP='INSERT' AND SESSION_USER='n6_virtual_executor' THEN
    RAISE EXCEPTION 'executor cannot create proposal';
$guard_executor_insert_078$;
  new_text := $guard_executor_insert_088$  ELSIF TG_OP='INSERT' AND SESSION_USER='n6_virtual_executor' THEN
    IF COALESCE((
...
$guard_executor_insert_088$;
  ...
  IF occurrence_count <> 1
    ...
    RAISE EXCEPTION '088_transition_guard_rewrite_scope_drift';
  END IF;
...
DECLARE
  ...
  before_summary jsonb := pg_catalog.current_setting(
    'n6.migration_088_business_summary', false
  )::jsonb;
...
BEGIN
  ...
  IF guard_proc.owner_name <> 'ashare_v3_user'
    ...
    RAISE EXCEPTION '088_postflight_definition_acl_or_privilege_drift';
  END IF;
  ...
  IF after_summary IS DISTINCT FROM before_summary THEN
    RAISE EXCEPTION '088_unexpected_business_dml';
  END IF;
END
$postflight$;
