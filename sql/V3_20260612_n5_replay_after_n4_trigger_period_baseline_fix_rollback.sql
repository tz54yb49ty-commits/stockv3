-- V3 20260612 N5 replay after N4 trigger-period baseline fix rollback.
...
\set action_run_id 'v3_n5_action_replay_20260612_after_n4_trigger_period_baseline_fix_v1'
\set source_trigger_run_id 'v3_n4_trigger_replay_20260612_after_trigger_period_baseline_fix_v1'
\set consumer_name 'v3_n5_action_replay_20260612_after_n4_trigger_period_baseline_fix_consumer_v1'
...
BEGIN
  ...
  IF v_count > 0 THEN
    RAISE EXCEPTION 'N5 fixed replay rollback blocked: scoped N5 outbox has delivered/delivering rows (%)', v_count;
  END IF;
  ...
  WHERE source_layer = 'N5_action'
    AND source_run_id = v_action_run_id;
  IF v_count > 0 THEN
    RAISE EXCEPTION 'N5 fixed replay rollback blocked: scoped N5 outbox has downstream inbox refs (%)', v_count;
  END IF;
  ...
  IF v_count > 0 THEN
    RAISE EXCEPTION 'N5 fixed replay rollback blocked: scoped N5 outbox has downstream checkpoint refs (%)', v_count;
  END IF;
  ...
  IF v_count > 0 THEN
    RAISE EXCEPTION 'N5 fixed replay rollback blocked: non-scoped consumer inbox refs for fixed N4 source (%)', v_count;
  END IF;
  ...
  LOOP
    ...
    IF v_table_regclass IS NOT NULL THEN
      ...
      IF v_count > 0 THEN
        RAISE EXCEPTION 'N5 fixed replay rollback blocked: downstream table % has scoped refs (%)', v_table_name, v_count;
      END IF;
    END IF;
  END LOOP;
END $$;
