-- N5 repaired-context action pipeline 20260605 rollback.
-- Scope:
...
-- Boundary:
--   Deletes only N5 action-layer rows and this N5 consumer's inbox/checkpoint rows
--   for the scoped N4 source run. It does not mutate N4 trigger facts/outbox,
--   N3 action-confirmation metric facts, N2 facts, N6/user/voice/mobile/sim/position,
--   virtual-account/order/trade/pnl, delivery/push, or real-trade facts.
...
BEGIN
  ...
  WITH scoped_n5_event_ids AS (
    ...
    UNION
    SELECT event_id FROM common_event_ledger WHERE source_layer = 'N5_action' AND source_run_id = v_action_run_id
  )
  SELECT count(*) INTO v_count
  FROM common_event_delivery_attempt
  WHERE event_id IN (SELECT event_id FROM scoped_n5_event_ids);
  IF v_count > 0 THEN
    RAISE EXCEPTION 'N5 rollback blocked: scoped N5 outbox has delivery attempt refs (%)', v_count;
  END IF;
  ...
  WITH scoped_n5_partitions AS (
    ...
    UNION
    SELECT DISTINCT partition_key FROM common_event_ledger WHERE source_layer = 'N5_action' AND source_run_id = v_action_run_id
  )
  SELECT count(*) INTO v_count
  ...
  WHERE run_id = v_action_run_id
    AND (
      ...
      OR user_layer_touched IS TRUE
      OR voice_touched IS TRUE
      OR sim_touched IS TRUE
      OR real_trade_touched IS TRUE
      OR COALESCE(raw_json::jsonb #>> '{boundary_proof,worker_started}', 'false') = 'true'
      OR COALESCE(raw_json::jsonb #>> '{boundary_proof,enter_n6}', 'false') = 'true'
      OR COALESCE(raw_json::jsonb #>> '{boundary_proof,user_voice_mobile_sim_position_real_trade_touched}', 'false') = 'true'
    );
  IF v_count > 0 THEN
    RAISE EXCEPTION 'N5 rollback blocked: scoped action_run records worker/downstream/user-layer touch (%)', v_count;
  END IF;
  ...
  LOOP
    ...
    IF v_table_regclass IS NOT NULL THEN
      ...
      IF v_count > 0 THEN
        RAISE EXCEPTION 'N5 rollback blocked: downstream/virtual/user table % has scoped refs (%)', v_table_name, v_count;
      END IF;
    END IF;
  END LOOP;
END $$;

-- Preview scoped rows immediately before deletion.
SELECT 'common_action_run' AS table_name, count(*) AS row_count FROM common_action_run WHERE run_id = :'action_run_id'
UNION ALL SELECT 'common_action_quality_item', count(*) FROM common_action_quality_item WHERE run_id = :'action_run_id'
UNION ALL SELECT 'stock_action_fact', count(*) FROM stock_action_fact WHERE run_id = :'action_run_id'
UNION ALL SELECT 'index_action_fact', count(*) FROM index_action_fact WHERE run_id = :'action_run_id'
UNION ALL SELECT 'board_action_fact', count(*) FROM board_action_fact WHERE run_id = :'action_run_id'
UNION ALL SELECT 'common_action_event', count(*) FROM common_action_event WHERE run_id = :'action_run_id'
UNION ALL SELECT 'common_event_outbox_n5', count(*) FROM common_event_outbox WHERE source_layer = 'N5_action' AND source_run_id = :'action_run_id'
UNION ALL SELECT 'common_event_ledger_n5', count(*) FROM common_event_ledger WHERE source_layer = 'N5_action' AND source_run_id = :'action_run_id'
UNION ALL SELECT 'common_event_inbox_n5_consumer', count(*) FROM common_event_inbox WHERE consumer_name = :'consumer_name' AND source_layer = 'N4_trigger' AND source_run_id = :'source_trigger_run_id';
...
DELETE FROM board_action_fact WHERE run_id = :'action_run_id';
DELETE FROM index_action_fact WHERE run_id = :'action_run_id';
DELETE FROM stock_action_fact WHERE run_id = :'action_run_id';
DELETE FROM common_action_quality_item WHERE run_id = :'action_run_id';
DELETE FROM common_action_run WHERE run_id = :'action_run_id';
