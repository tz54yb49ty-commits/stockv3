-- V3 20260615 N6 user projection rollback.
...
SET LOCAL n6.rollback_user_projection_run_id = 'v3_n6_user_projection_20260615_after_n5_action_bounded_20260615_from_n4_production_semantic_replay_20260615_market_snapshot_updated_until_1000';
...
BEGIN
  IF v_run_id IS NULL OR btrim(v_run_id) = '' THEN
    RAISE EXCEPTION 'V3 20260615 N6 rollback blocked: n6.rollback_user_projection_run_id is not set';
  END IF;

  IF v_run_id <> 'v3_n6_user_projection_20260615_after_n5_action_bounded_20260615_from_n4_production_semantic_replay_20260615_market_snapshot_updated_until_1000' THEN
    RAISE EXCEPTION 'V3 20260615 N6 rollback blocked: unexpected run id %', v_run_id;
  END IF;
  ...
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'V3 20260615 N6 rollback blocked: expected 1 user_projection_run row for %, found %', v_run_id, v_count;
  END IF;
  ...
  IF v_source_action_run_id <> 'n5_action_bounded_20260615_from_n4_production_semantic_replay_20260615_market_snapshot_updated_until_1000' THEN
    RAISE EXCEPTION 'V3 20260615 N6 rollback blocked: unexpected source_action_run_id % for run %', v_source_action_run_id, v_run_id;
  END IF;
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'V3 20260615 N6 rollback blocked: user_signal_decision has % rows linked to run %', v_count, v_run_id;
  END IF;
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'V3 20260615 N6 rollback blocked: user_sim_order has % rows linked to run %', v_count, v_run_id;
  END IF;

  SELECT count(*) INTO v_count FROM user_sim_trade WHERE sim_run_id = v_run_id;
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'V3 20260615 N6 rollback blocked: user_sim_trade has % rows linked to run %', v_count, v_run_id;
  END IF;

  SELECT count(*) INTO v_count FROM user_sim_position WHERE sim_run_id = v_run_id;
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'V3 20260615 N6 rollback blocked: user_sim_position has % rows linked to run %', v_count, v_run_id;
  END IF;
  ...
  LOOP
    ...
    IF v_count <> 0 THEN
      RAISE EXCEPTION 'V3 20260615 N6 rollback blocked: linked % refs found in %, rows=%, run=%', v_guard.category, v_guard.table_name, v_count, v_run_id;
    END IF;
  END LOOP;
  ...
  IF v_deleted_runs <> 1 THEN
    RAISE EXCEPTION 'V3 20260615 N6 rollback blocked: expected to delete 1 projection run, deleted %', v_deleted_runs;
  END IF;

  RAISE NOTICE 'V3 20260615 N6 rollback completed for %, notification_rows=%, card_rows=%, projection_rows=%, run_rows=%',
