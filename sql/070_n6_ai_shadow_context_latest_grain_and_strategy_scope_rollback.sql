...
-- Restore exact pre-070 functions without deleting snapshots or audit history.
...
DECLARE
  ...
  actual_sha text;
  schedule_oid oid;
  schedule_proc record;
  schedule_role_oid oid;
  error_prefix text := '070_rollback_requires_fixed_state';
BEGIN
  IF SESSION_USER <> 'ashare_v3_user'
     OR CURRENT_USER <> 'ashare_v3_user' THEN
    RAISE EXCEPTION '070_rollback_owner_session_required';
  END IF;

  schedule_oid := pg_catalog.to_regprocedure(
    'public.n6_ai_agent_shadow_schedule_preflight(text,date)'
  );
  IF schedule_oid IS NULL THEN
    RAISE EXCEPTION '070_rollback_requires_067_schedule';
  END IF;
  ...
  WHERE function_row.oid = schedule_oid;
  SELECT role.oid INTO schedule_role_oid
  FROM pg_catalog.pg_roles role
  WHERE role.rolname = 'n6_ai_agent';
  IF schedule_proc.owner_name IS DISTINCT FROM 'ashare_v3_user'
     OR schedule_proc.language_name IS DISTINCT FROM 'plpgsql'
     OR schedule_proc.prosecdef IS DISTINCT FROM true
     OR schedule_proc.proisstrict IS DISTINCT FROM false
     OR schedule_proc.proleakproof IS DISTINCT FROM false
     OR schedule_proc.provolatile IS DISTINCT FROM 's'
     OR schedule_proc.proparallel IS DISTINCT FROM 'u'
     OR schedule_proc.proconfig IS DISTINCT FROM
          ARRAY['search_path=pg_catalog']::text[]
     OR schedule_proc.source_sha256 IS DISTINCT FROM
          '1ec882400c5cb95e1743e7f8829327d6cf42e3bfb7ea68a64c70795a1d73731d'
     OR schedule_role_oid IS NULL
     OR (
       SELECT pg_catalog.count(*)
       FROM pg_catalog.aclexplode(
         COALESCE(
           schedule_proc.proacl,
           pg_catalog.acldefault('f', schedule_proc.owner_oid)
         )
       ) function_acl
     ...
     OR NOT EXISTS (
       ...
       WHERE function_acl.grantee = schedule_proc.owner_oid
     ...
     OR NOT EXISTS (
       ...
       WHERE function_acl.grantee = schedule_role_oid
         AND function_acl.privilege_type = 'EXECUTE'
         AND NOT function_acl.is_grantable
     )
     OR EXISTS (
  ...
  FOR expected IN
    ...
    ) AS expected_functions(signature, fixed_sha, allowed_role)
  LOOP
    function_oid := pg_catalog.to_regprocedure(
      'public.' || expected.signature
    );
    IF function_oid IS NULL THEN
      RAISE EXCEPTION '070_rollback_requires_fixed_state: %',
        expected.signature;
    END IF;
    ...
    IF actual_sha <> expected.fixed_sha THEN
      RAISE EXCEPTION '070_rollback_requires_fixed_state: body %',
        expected.signature;
    END IF;
  END LOOP;
END
...
DECLARE
  ...
  allowed_role_oid oid;
  actual_sha text;
  error_prefix text := '070_rollback_postflight_mismatch';
BEGIN
  FOR expected IN
    SELECT *
    FROM (
      ...
      ('n6_ai_agent_context_load_v2(text,date,integer,text)', 'df2afc2d7583effd10905ed478ab0df7e2147a854784bfc1b6087ca6d9b04681', 'n6_ai_agent'),
      ('n6_ai_strategy_context_load_v1(text,date,integer,text)', '79dd370a27ff53b270ab032542cb0fc4eed3262a673919ff8f0d6e751592f504', NULL::text)
  ...
  LOOP
    ...
    IF function_oid IS NULL THEN
      RAISE EXCEPTION '070_rollback_postflight_mismatch: %', expected.signature;
    END IF;
    SELECT function_row.prosrc, function_row.prosecdef,
           function_row.proisstrict, function_row.proleakproof,
           function_row.provolatile, function_row.proparallel,
    ...
    IF actual_sha <> expected.expected_sha THEN
      RAISE EXCEPTION '070_rollback_postflight_mismatch: body %', expected.signature;
    END IF;
  END LOOP;
END
$postflight$;
