-- N3 action-confirmation metric 20260608 until 1500 materialization business rollback.
-- Scope: delete only rows for projection_run_id=action_confirmation_metric_20260608_until_1500__trigger_projection_matcher_execute_20260608_v13_index_all_until_1500_v4_repair_retry.
...
\set projection_run_id 'action_confirmation_metric_20260608_until_1500__trigger_projection_matcher_execute_20260608_v13_index_all_until_1500_v4_repair_retry'
...
DECLARE
  ...
  outbox_refs BIGINT;
  delivered_or_delivering_outbox_refs BIGINT;
  inbox_refs BIGINT;
...
BEGIN
  ...
  IF outbox_refs <> 0 OR delivered_or_delivering_outbox_refs <> 0 OR inbox_refs <> 0 OR checkpoint_refs <> 0
     OR trigger_refs <> 0 OR action_refs <> 0 OR n6_refs <> 0
     OR touched_refs <> 0 OR worker_refs <> 0 THEN
    RAISE EXCEPTION
      'N3 action metric rollback blocked for %, outbox=%, delivered_or_delivering=%, inbox=%, checkpoint=%, trigger=%, action=%, n6=%, downstream_touched=%, worker=%',
      target_run_id, outbox_refs, delivered_or_delivering_outbox_refs, inbox_refs, checkpoint_refs, trigger_refs, action_refs, n6_refs, touched_refs, worker_refs;
  END IF;
END $$;
