-- N6 B-track manual actionable buy support for targetless N6 projections.
-- REVIEW ONLY: do not execute migration, send proposals, or start the executor in this gate.
...
BEGIN
  ...
  IF proposal.proposal_side = 'buy' THEN
    ...
    IF (
      ...
      RETURN pg_catalog.jsonb_build_object(
        'ok', false, 'status', 'signal_projection_lineage_not_ready'
      );
    END IF;
  ...
  INSERT INTO public.n6_virtual_cash_ledger (
    ...
    p_executor_run_id, 'n6_btrack_manual_actionable_buy_063_v1',
    'n6_btrack_manual_actionable_buy_063_v1',
  ...
  IF position_before.virtual_position_id IS NULL THEN
    INSERT INTO public.n6_virtual_position (
      ...
      p_executor_run_id, 'n6_btrack_manual_actionable_buy_063_v1',
      'n6_btrack_manual_actionable_buy_063_v1',
      p_executor_run_id, lineage, 'passed', episode_no, trade_date_date,
      proposal.locked_target_price,
      CASE
        WHEN proposal.locked_target_price IS NOT NULL
         AND proposal.locked_target_price > 0 THEN 'frozen'
        ELSE 'not_ready'
      END,
      CASE
        WHEN proposal.locked_target_price IS NOT NULL
         AND proposal.locked_target_price > 0
          THEN proposal.source_signal_projection_id
        ELSE NULL
      END,
      'provisional_first_day'
  ...
  ELSE
    ...
    SET position_status = CASE WHEN new_quantity = 0 THEN 'closed_virtual' ELSE 'open_virtual' END,
        ...
        policy_version = 'n6_btrack_manual_actionable_buy_063_v1',
        policy_hash = 'n6_btrack_manual_actionable_buy_063_v1',
