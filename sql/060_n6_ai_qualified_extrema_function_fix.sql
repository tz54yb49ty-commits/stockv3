...
-- Repair PostgreSQL special expressions in five published N6 AI functions.
-- Function signatures, business logic, owner and ACL are unchanged.
...
BEGIN
  IF SESSION_USER <> 'ashare_v3_user'
    ...
    RAISE EXCEPTION '060_owner_session_required';
  END IF;
  ...
  FOR expected IN
    ...
    FROM (
      VALUES
      ('n6_ai_agent_daily_summary_record(jsonb)', 'c8e0928d3afb20535792a82b20270d32d66d466a85d0d112aefc64fa8f573a5e', '235b3913734fda03a3d55822d58f785ae3dad36002c38f473bfeaad6636f0042'),
      ('n6_ai_agent_context_load(text,date,integer)', 'bbcd60822e8d18e6731ac0f46e68d2dd545dea08a172f28655eead4e1444fa84', '4dae0563b34df9e066c2c91feb6f3a096a09ea2573a31f2cf30c71bfe0704993'),
      ('n6_ai_agent_proposal_create_confirm(jsonb)', '2bde0f7d24cd88bc2851fb162b8499730560e8959c8f814d6625a97fe9db063b', 'aa3806a66ed5fa08b3c497e42cfb0142c61759b796891cf81d7c041024de05f2'),
      ('n6_ai_executor_risk_recheck(bigint,text)', '51faf7163dd35ead8f290c7a0ec17849f56dd0906339b8de4b7f1c5c932e834c', 'f42d6750d192321f851626428589fdc342355410b7e1c50a33855642661bbf75'),
      ('n6_ai_strategy_shadow_evaluate(date,text,text)', 'a7cd3200d0c4a226c9ea03fc14e62a03f86877f768d991911eafc8b7a13c2cb2', 'fcd1ada453c672c8a2caa5caa4857b15f7d162f2ed5780cda27c7cd41ad6b474')
    ) AS expected_functions(signature, source_sha, fixed_sha)
  LOOP
    function_oid := pg_catalog.to_regprocedure('public.' || expected.signature);
    IF function_oid IS NULL THEN
      RAISE EXCEPTION '060_partial_or_source_mismatch: %', expected.signature;
    END IF;
    ...
    JOIN pg_catalog.pg_roles function_owner ON function_owner.oid = function_row.proowner
    ...
    IF NOT (
      ...
      RAISE EXCEPTION '060_partial_or_source_mismatch: attributes %', expected.signature;
    END IF;
    IF pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(function_proc.prosrc, 'UTF8')), 'hex') = expected.source_sha THEN
      source_count := source_count + 1;
    ELSIF pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(function_proc.prosrc, 'UTF8')), 'hex') = expected.fixed_sha THEN
      fixed_count := fixed_count + 1;
    ELSE
      RAISE EXCEPTION '060_partial_or_source_mismatch: body %', expected.signature;
    END IF;
  END LOOP;
  IF fixed_count = 5 THEN
    RAISE EXCEPTION '060_already_applied';
  END IF;
  IF source_count <> 5 OR fixed_count <> 0 THEN
    RAISE EXCEPTION '060_partial_or_source_mismatch';
  END IF;
END
$preflight$;
...
BEGIN
  FOR expected IN
    ...
    FROM (
      VALUES
      ('n6_ai_agent_daily_summary_record(jsonb)', '235b3913734fda03a3d55822d58f785ae3dad36002c38f473bfeaad6636f0042'),
      ('n6_ai_agent_context_load(text,date,integer)', '4dae0563b34df9e066c2c91feb6f3a096a09ea2573a31f2cf30c71bfe0704993'),
      ('n6_ai_agent_proposal_create_confirm(jsonb)', 'aa3806a66ed5fa08b3c497e42cfb0142c61759b796891cf81d7c041024de05f2'),
      ('n6_ai_executor_risk_recheck(bigint,text)', 'f42d6750d192321f851626428589fdc342355410b7e1c50a33855642661bbf75'),
      ('n6_ai_strategy_shadow_evaluate(date,text,text)', 'fcd1ada453c672c8a2caa5caa4857b15f7d162f2ed5780cda27c7cd41ad6b474')
    ) AS expected_functions(signature, expected_sha)
  LOOP
    function_oid := pg_catalog.to_regprocedure('public.' || expected.signature);
    IF function_oid IS NULL THEN
      RAISE EXCEPTION '060_postflight_mismatch: %', expected.signature;
    END IF;
    SELECT function_row.prosrc, function_row.prosecdef,
           function_row.proisstrict, function_row.proleakproof,
           function_row.provolatile, function_row.proparallel,
